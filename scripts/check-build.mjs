...
for(const token of ['def normalized_text(', 'def select_uses_dark_theme(', 'class QuietStaticHandler(', 'def static_server(', 'ThreadingHTTPServer', 'checks.capture_failure(page, "public-desktop-failure", runtime_events)', 'page.on("pageerror"', 'page.on("requestfailed"', 'hero_limit = min(700', '#bookingForm header [data-close]', 'Saturday morning to Sunday morning keeps 1000 UAH deposit', 'Saturday evening to Sunday evening keeps 1000 UAH deposit', 'Friday evening to Sunday morning uses 2000 UAH weekend deposit', 'Friday evening to Sunday evening uses 2000 UAH weekend deposit', 'Saturday morning to Monday morning uses 2000 UAH weekend deposit'])if(!e2eSmoke.includes(token))errors.push(`E2E CI hardening missing: ${token}`);
