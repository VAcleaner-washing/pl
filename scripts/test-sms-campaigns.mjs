...
check(phonePromo.includes('vacleaner_sms_dispatch_recipients')&&phonePromo.includes('vacleaner_sms_dispatches')&&phonePromo.includes('codeFromMessage')&&phonePromo.includes('vacleaner_promo_redemptions'),'phone promo lookup is tied to an actual sent dispatch and rejects expired or already-used codes');
check(!bookingBridge.includes('http-equiv="refresh"')&&bookingBridge.includes("params.set('promo',raw)")&&bookingBridge.includes("location.replace('/bronuvannia/'+(query?'?'+query:''))"),'short /b bridge converts the personalized fragment into a durable promo query without a racing meta refresh');
