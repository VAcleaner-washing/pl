...
check(admin.includes('давно не освіжали дім? 😊')&&admin.includes('на оренду')&&admin.includes('Активуйте бонус: {link} Діє 21 день'),'RETURN default copy is universal and activation-based');
const returnPreview='VAcleaner: давно не освіжали дім? 😊 -10% на оренду. Активуйте бонус: vacleaner.pp.ua/b#XXXXXXX Діє 21 день. Стоп: vacleaner.pp.ua/s';
...
check(phonePromo.includes('vacleaner_sms_dispatch_recipients')&&phonePromo.includes('vacleaner_sms_dispatches')&&phonePromo.includes('qualifyingDispatch')&&phonePromo.includes('vacleaner_promo_redemptions')&&phonePromo.includes('.eq("active",true)'),'phone promo lookup is tied to issued SMS and returns only active unused bonuses');
check(!bookingBridge.includes('http-equiv="refresh"')&&bookingBridge.includes("action:'activate'")&&bookingBridge.includes("params.set('promo'")&&bookingBridge.includes("location.replace('/bronuvannia/'"),'short /b bridge activates the personalized bonus before forwarding it into booking');
...
check(admin.includes('data-sms-details')&&admin.includes("SMS_AUDIT_API=SUPABASE+'/functions/v1/vacleaner-sms-audit-v1'")&&admin.includes("action:'dispatch_recipients'")&&smsAudit.includes('vacleaner_sms_dispatch_recipients'),'SMS journal drills into a narrow authenticated recipient-audit Edge Function without changing send transport');
check(admin.includes('sms-dispatch-promo')&&admin.includes('row.promo_code')&&admin.includes('row.customer_phone'),'SMS dispatch detail exposes recipient phone, delivery state and promo code');
check(adminCss.includes('v4.1.00 — SMS JOURNAL + RECIPIENT DRILL-DOWN')&&adminCss.includes('.sms-history-open span')&&adminCss.includes('.sms-history-actions'),'Journal badge stays inside one control and mobile history actions have deliberate geometry');
