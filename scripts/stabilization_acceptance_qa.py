...
def setup_fixture():
    ...
    b.update({
      ...
      'base_amount':1200,'delivery_amount':250,'extras_amount':450,'total_amount':1900,
      ...
      'extras':{'selected_items':[{'code':'premium_sc2','label':'Насадки «Преміум» до SC 2','price':200},{'code':'odour_zero','label':'Odour Zero','price':250}],'base_before_discount':1200,'discount':{'percent':0,'amount':0,'source':'none'},'chemistry':{'used_packets':2,'story_mention':True,'free_packets':2,'paid_packets':0,'price_per_packet':50,'amount':0},'settlement':{'refund_amount':250,'due_amount':0,'completed':True}}
    })
    return b
...
try:
  ...
  with sync_playwright() as p:
    ...
    try:
      ...
      extra_row=page.locator('#live .finance-extra-row')
      check(extra_row.count()==1 and '450 грн' in extra_row.inner_text(),'desktop: additional total remains 450 грн')
      check('Насадки 200 грн + Засоби 250 грн' in extra_row.inner_text(),'desktop: additional total explains nozzles and supplies')
    ...
    try:
      ...
      card.locator('[data-action="finance"]').click(); page.wait_for_selector('#financeForm')
      extra_row=page.locator('#live .finance-extra-row')
      check(extra_row.count()==1 and 'Насадки 200 грн + Засоби 250 грн' in extra_row.inner_text(),'mobile-390: additional breakdown remains visible and readable')
      m=modal_metrics(page)
