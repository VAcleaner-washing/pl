...
def modal_metrics(page):
    return page.locator('#financeForm').evaluate('''form=>{const q=s=>form.querySelector(s),r=e=>e.getBoundingClientRect(),labels=[...form.querySelectorAll('.return-story-options>label')],texts=[...form.querySelectorAll('.return-story-options b')];const card=form.closest('.modal-card'),layout=q('.modal-layout'),left=q('.modal-section'),fields=q('.fields'),gift=q('.return-story-bonus'),summary=q('.modal-summary'),footer=q('footer');return{viewport:[innerWidth,innerHeight],card:r(card),layout:r(layout),left:r(left),fields:r(fields),gift:r(gift),summary:r(summary),footer:r(footer),labels:labels.map(r),textOverflow:texts.map(x=>x.scrollWidth>x.clientWidth+1),horizontal:card.scrollWidth-card.clientWidth,layoutHorizontal:layout.scrollWidth-layout.clientWidth}}''')
...
try:
  ...
  with sync_playwright() as p:
    ...
    try:
      ...
      check(m['summary']['height'] < m['layout']['height']*0.86,'desktop: summary no longer stretches into a tall empty card')
