...
articles=[
('yak-vyvesty-plyamu-z-dyvana','Як працювати з плямою на дивані — VAcleaner','Що робити з плямою на дивані до глибокого промивання, як не рознести забруднення і коли потрібні VA SPOT FIX, VA STAIN OX та Puzzi.',
'''<p class="v4-lead">Пляма — це не завжди задача «потерти сильніше». Найчастіше результат залежить від того, чи не втерли забруднення глибше ще до основного очищення.</p><h2>1. Не розтирайте пляму по площі</h2><p>Якщо забруднення свіже, спочатку промокніть надлишок чистою серветкою або мікрофіброю. Рухайтесь від країв до центру.</p><h2>2. Зрозумійте, що це за пляма</h2><p>Для плям від їжі, жиру, косметики або невідомого походження використовуйте універсальний плямовивідник VA SPOT FIX 50 мл. Для стійких плям від напоїв і харчових продуктів — кави, чаю, червоного вина, соків або ягід — використовуйте VA STAIN OX 30 мл. Якщо потрібні обидва засоби, їх застосовують окремо з промиванням між етапами. Перед першим застосуванням обов’язково перевіряють стійкість барвника на непомітній ділянці.</p><h2>3. Плямовивідник — не заміна промиванню</h2><p>Після локального засобу оброблену ділянку потрібно промити чистою водою або миючим пилососом. Для диванів і крісел цю роль виконує Kärcher Puzzi.</p><h2>4. Якщо є ще й запах</h2><p>Пляма й запах — дві різні задачі. Якщо мова про запах сечі, у VAcleaner пріоритетний нейтралізатор — Neutralix; його не варто підміняти звичайним ароматизатором.</p>'''),
...
('yak-pochystyty-matrats-pislia-dytyny','Як почистити матрац після дитини — пляма та запах сечі | VAcleaner','Послідовність очищення матраца після дитини: сухий етап, пляма, запах сечі, Neutralix, VA SPOT FIX та глибоке промивання Puzzi.',
'''<p class="v4-lead">У матраца після дитини часто дві окремі проблеми: видима пляма та запах. Якщо працювати лише з однією, результат може бути неповним.</p><h2>1. Почніть із сухого етапу</h2><p>Перед вологим очищенням приберіть пил, волосся й дрібне сухе сміття. Якщо пилу багато, особливо в матраці, окремий сухий етап помітно спрощує подальше промивання.</p><h2>2. Запах сечі — окрема задача</h2><p>У підборі VAcleaner для запаху сечі Neutralix має пріоритет №1. Його задача — нейтралізація запаху, а не маскування ароматом.</p><h2>3. Якщо лишилась видима пляма</h2><p>Тоді додається універсальний плямовивідник VA SPOT FIX 50 мл — для локального опрацювання плям від їжі, жиру, косметики або невідомого походження. Перед застосуванням перевірте стійкість барвника на непомітній ділянці.</p><h2>4. Завершіть глибоким промиванням</h2><p>Kärcher Puzzi промиває текстиль і відбирає забруднену воду. Після очищення матрацу потрібно дати повністю висохнути з нормальною вентиляцією.</p>''')]
...
function privacyConsent(){if(path!=='/bronuvannia')return;const span=document.querySelector('.booking-consent:not(.vx-marketing-consent) > span');if(!span||span.querySelector('a[href="/polityka-konfidenciynosti/"]'))return;const terms=span.querySelector('a[href^="/umovy"]');if(!terms)return;const a=document.createElement('a');a.href='/polityka-konfidenciynosti/';a.target='_blank';a.rel='noopener';a.textContent='політику конфіденційності';terms.insertAdjacentText('afterend',' та ');terms.insertAdjacentElement('afterend',a);}
