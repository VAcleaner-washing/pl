...
function bookingHelp(){if(path!=='/bronuvannia'||document.querySelector('.v4-booking-help'))return;const heading=document.querySelector('#booking-products .booking-step-heading');if(!heading)return;const box=document.createElement('div');box.className='v4-booking-help';box.innerHTML='<span>Не впевнені у виборі?</span><a href="/pidbir/">Підбір за ~30 секунд →</a>';heading.insertAdjacentElement('afterend',box);}
...
function boot(){patchNav();patchFooter();mobileMenu();reviewsProof();termsDelivery();bookingHelp();vahomeBridge()}
