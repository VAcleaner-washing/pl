...
const allSelectsCovered=selectPositions.length>=13&&selectPositions.every(index=>{const prefix=js.slice(Math.max(0,index-260),index);const tail=js.slice(index,index+90);return prefix.includes('class="field"')||prefix.includes('clients-toolbar')||prefix.includes('campaign-product-field')||prefix.includes('finance-ledger-head')||prefix.includes('sms-audience-sort')||prefix.includes('sms-toolbar-field')||tail.includes('id="clientSegment"')||tail.includes('id="clientSort"')});
