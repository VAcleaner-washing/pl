...
const allSelectsCovered=selectPositions.length>=13&&selectPositions.every(index=>{const prefix=js.slice(Math.max(0,index-420),index);const tail=js.slice(index,index+90);return prefix.includes('class="field')||prefix.includes('clients-toolbar')||prefix.includes('campaign-product-field')||prefix.includes('finance-ledger-head')||prefix.includes('sms-audience-sort')||prefix.includes('sms-toolbar-field')||tail.includes('id="clientSegment"')||tail.includes('id="clientSort"')});
const allCheckboxesCovered=checkboxPositions.length>=13&&checkboxPositions.every(index=>{const prefix=js.slice(Math.max(0,index-420),index);return prefix.includes('class="switch')||prefix.includes('class="extra-check')||prefix.includes('class="sms-recipient')||prefix.includes('class="sms-legacy-attestation')});
const checks=[
 ...
 ['desktop client card keeps structured glass layout',js.includes('client-editor-summary')&&js.includes('client-card-grid')&&js.includes('client-contact-section')&&js.includes('client-document-section')&&js.includes('client-history-section')&&css.includes('v4.0.9 — desktop client card')&&css.includes('grid-template-columns:minmax(310px,.82fr) minmax(430px,1.16fr) minmax(300px,.82fr)')],
 ['desktop client quick actions are actually styled on desktop',glassCss.includes('v4.0.9 — desktop client card geometry')&&glassCss.includes('@media (min-width:901px){\n  html.glass-test .glass-client-actions')&&glassCss.includes('grid-template-columns:repeat(3,minmax(0,1fr))')],
