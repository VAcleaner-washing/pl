...
CSS=['admin-v250.css','admin-glass-test.css','address-autocomplete.css','admin-native-v2.css','admin-native-v21.css','admin-native-v22.css','admin-native-v23.css']
JS=['vacleaner-core.js','admin-v250.js','admin-glass-test.js','address-autocomplete.js','admin-native-v2.js','admin-native-v22.js','admin-native-v23.js']
passed=0;failed=[]
def check(ok,label):
    global passed
...
def boot(browser,width=390,height=844,add_cancelled=False):
    ...
    p.evaluate("html=>{document.body.innerHTML=html;document.documentElement.classList.add('glass-test','native-test','native-v2','native-v21','native-v22','native-v23')}",INITIAL)
...
with sync_playwright() as pw:
    ...
    # More keeps the connection state without restoring topbar clutter.
    p=boot(b);p.locator('.mobile-nav .more-nav').click();p.wait_for_selector('.mobile-more-menu');p.wait_for_timeout(70)
    conn=p.locator('.native-v23-connection:visible');check(conn.count()==1 and 'Онлайн' in conn.inner_text(),'More: online state exposed')
    ...
    check('Офлайн' in conn.inner_text() and 'offline' in (conn.get_attribute('class') or ''),'More: offline state updates')
...
manifest=json.loads((ROOT/'admin/manifest-native-v23.webmanifest').read_text())
check(manifest['name']=='VAcleaner Native V2.3 Final RC','Manifest: V2.3 name')
check(manifest['start_url']=='/admin/bronuvannia-native-v23/' and manifest['scope']=='/admin/bronuvannia-native-v23/','Manifest: V2.3 route/scope')
sw=(ROOT/'admin/sw-native-v23.js').read_text()
check("const FALLBACK=ROUTE" in sw,'SW: offline fallback stays on V2.3')
check("/^\\/admin\\/bronuvannia\\/?$/" in sw and "target.pathname=ROUTE" in sw,'SW: production deep links rewrite to V2.3')
route=(ROOT/'admin/bronuvannia-native-v23/index.html').read_text()
check('/assets/admin-native-v23.css?v=4247' in route and '/assets/admin-native-v23.js?v=4247' in route,'Route: V2.3 assets loaded')

print(f'NATIVE V2.3 QA: PASS {passed} · FAIL {len(failed)}')
if failed:
