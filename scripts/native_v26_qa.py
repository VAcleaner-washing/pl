...
manifest=json.loads((ROOT/'admin/manifest-native-v26.webmanifest').read_text())
check(manifest['name']=='VAcleaner Native V2.6 Final Polish RC','Manifest: V2.6 name')
check(manifest['start_url']=='/admin/bronuvannia-native-v26/' and manifest['scope']=='/admin/bronuvannia-native-v26/','Manifest: V2.6 route/scope')
sw=(ROOT/'admin/sw-native-v26.js').read_text()
check("const FALLBACK=ROUTE" in sw,'SW: offline fallback stays on V2.6')
check("/^\\/admin\\/bronuvannia\\/?$/" in sw and "target.pathname=ROUTE" in sw,'SW: production deep links rewrite to V2.6')
...
check("const ROUTE='/admin/bronuvannia-native-v26/'" in v25js and 'data:{url:ROUTE}' in v25js and '__nativeV26=true' in v25js,'Local notifications: V2.6 route wins over inherited V2.3 patch')

print(f'NATIVE V2.6 QA: PASS {passed} · FAIL {len(failed)}')
if failed:
