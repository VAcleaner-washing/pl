...
def public_tests(browser: Browser, base: str, api_handler, checks: Checks, static_root: Path) -> None:
    ...
    try:
        ...
        checks.check("is-va-stain-care" in (spot.get_attribute("class") or "") and "is-va-stain-care" in (stain.get_attribute("class") or ""), "VA spot-care cards carry their branded class before selection")
        spot_before = page.evaluate("e=>({bg:getComputedStyle(e).backgroundImage,badge:getComputedStyle(e.querySelector('b'),'::before').content})", spot.element_handle())
        spot.locator('input[type="checkbox"]').check()
        page.wait_for_timeout(80)
        spot = page.locator(".booking-extras label", has_text="VA SPOT FIX")
        spot_after = page.evaluate("e=>({cls:e.className,bg:getComputedStyle(e).backgroundImage,badge:getComputedStyle(e.querySelector('b'),'::before').content})", spot.element_handle())
        checks.check("is-selected" in spot_after["cls"] and "is-va-stain-care" in spot_after["cls"], "Selected VA SPOT FIX keeps both selected and branded classes")
        checks.check("VA PROFESSIONAL SPOT CARE" in spot_after["badge"], "Selected VA SPOT FIX keeps the VA PROFESSIONAL SPOT CARE badge")
        checks.check(spot_after["bg"] != "none", "Selected VA SPOT FIX keeps its branded background")
        spot.locator('input[type="checkbox"]').uncheck()
    ...
    try:
        ...
        address = page.locator('#booking-extras .booking-delivery-address input:not([data-vac-address-detail])')
        address.fill('Європейська 146Е')
        address_detail = page.locator('#booking-extras .booking-delivery-address input[data-vac-address-detail]')
        checks.check(address_detail.count() == 1, "Delivery access note has its own input")
        address_detail.fill('2 під’їзд, зі сторони двору')
        page.wait_for_timeout(50)
...
def main() -> int:
    ...
    with static_server(root) as base, sync_playwright() as playwright:
        ...
        # Playwright-managed Chromium is the reproducible default. A system Chromium
        # may carry enterprise URL-blocking policies and produced false local failures.
        if executable:
