...
def boot(browser,width=390,height=844,add_cancelled=False):
    ...
    p.evaluate("html=>{document.body.innerHTML=html;document.documentElement.classList.add('glass-test','native-test','native-v2','native-v21','native-v22','native-v23','native-v24')}",INITIAL)
...
with sync_playwright() as pw:
    ...
    # V2.4 screenshot-derived visual contracts.
    p=boot(b);open_view(p,'bookings')
    toolbar=p.locator('.booking-toolbar')
    check(toolbar.count()==1 and toolbar.evaluate("e=>getComputedStyle(e).position==='static'"),'V2.4: booking filters scroll with content, no card overlap')
    chips=p.locator('.booking-toolbar .chip:visible')
    check(all(x>=44 for x in chips.evaluate_all("els=>els.map(e=>e.getBoundingClientRect().height)")),'V2.4: booking filter targets 44px+')
    p.locator('[data-filter="completed"]').click();p.wait_for_timeout(60)
    compact=p.locator('.booking-card.native-v22-compact').first
    if compact.count():
        check(compact.locator('.booking-finance>em.neutral:visible').count()==0,'V2.4: zero final balance does not waste compact-card row')
    p.close()
    ...
    if entry.count():
        check(entry.evaluate("e=>getComputedStyle(e).borderRadius==='0px'"),'V2.4: audit history rows are flat')
    reload=p.locator('#auditReload:visible')
    if reload.count():check(reload.evaluate("e=>e.getBoundingClientRect().height>=44"),'V2.4: audit reload 44px touch target')
    ...
    if more.count():
        ...
        check(actions.count()>=1 and all(x>=50 for x in actions.evaluate_all("els=>els.map(e=>e.getBoundingClientRect().height)")),'V2.4: action sheet uses full-height list rows')
    p.close()

    p=boot(b);p.locator('.mobile-nav .more-nav').click();p.wait_for_selector('.mobile-more-menu');p.wait_for_timeout(50)
    check(p.locator('.native-v23-connection:visible').count()==0,'V2.4: standalone connection row removed')
    meta=p.locator('.native-profile-card span>small:visible')
    check(meta.count()==1 and 'Адміністратор' in meta.inner_text(),'V2.4: profile metadata retained')
    p.close()
    ...
    for action,formsel in [('issue','.issue-form'),('finance','.finance-form')]:
        ...
        if loc.count():
            ...
            check(head.count()==1 and head.evaluate("e=>parseFloat(getComputedStyle(e).borderTopWidth)<=1"),f'V2.4: {action} context has one thin shell')
        p.close()

    p=boot(b);open_view(p,'campaigns')
    sm=p.locator('.campaign-metrics small:visible')
    if sm.count():check(min(sm.evaluate_all("els=>els.map(e=>parseFloat(getComputedStyle(e).fontSize))"))>=10,'V2.4: campaign metric labels readable')
    p.close()

    p=boot(b);open_view(p,'calendar')
    labels=p.locator('.day-labels>span:visible')
    if labels.count():check(min(labels.evaluate_all("els=>els.map(e=>parseFloat(getComputedStyle(e).fontSize))"))>=10.5,'V2.4: calendar column labels readable')
    p.close()
    b.close()

# Static PWA isolation/deep-link contracts.
manifest=json.loads((ROOT/'admin/manifest-native-v24.webmanifest').read_text())
check(manifest['name']=='VAcleaner Native V2.4 Visual Polish RC','Manifest: V2.4 name')
check(manifest['start_url']=='/admin/bronuvannia-native-v24/' and manifest['scope']=='/admin/bronuvannia-native-v24/','Manifest: V2.4 route/scope')
sw=(ROOT/'admin/sw-native-v24.js').read_text()
check("const FALLBACK=ROUTE" in sw,'SW: offline fallback stays on V2.4')
check("/^\\/admin\\/bronuvannia\\/?$/" in sw and "target.pathname=ROUTE" in sw,'SW: production deep links rewrite to V2.4')
route=(ROOT/'admin/bronuvannia-native-v24/index.html').read_text()
check('/assets/admin-native-v24.css?v=4247' in route and '/assets/admin-native-v24.js?v=4247' in route,'Route: V2.4 assets loaded')
v24js=(ROOT/'assets/admin-native-v24.js').read_text()
check("const ROUTE='/admin/bronuvannia-native-v24/'" in v24js and 'data:{url:ROUTE}' in v24js and '__nativeV23=true' in v24js,'Local notifications: V2.4 route wins over inherited V2.3 patch')

print(f'NATIVE V2.4 QA: PASS {passed} · FAIL {len(failed)}')
if failed:
