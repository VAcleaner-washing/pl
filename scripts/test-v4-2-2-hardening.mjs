...
ok('release preserves v4.2.2 hardening baseline',String(rel.version).startsWith('4.2.')&&Number(rel.build)>=4202&&pkg.version===rel.version);
...
const runtimeLoader=read('assets/public-runtime-loader.js'),smartGuideFit=read('scripts/test-smart-guide-fit.py'),adminCss=read('assets/admin-v250.css');
ok('route runtime loads canonical core before experience runtime',runtimeLoader.indexOf("load('/assets/vacleaner-core.js'")>=0&&runtimeLoader.indexOf("load('/assets/vacleaner-core.js'")<runtimeLoader.indexOf("load('/assets/public-experience-runtime.js'"));
ok('Smart Guide browser QA boots canonical core before quiz',smartGuideFit.includes("CORE_JS=(ROOT/'assets/vacleaner-core.js')")&&smartGuideFit.includes('add_script_tag(content=CORE_JS);page.add_script_tag(content=JS)'));
ok('campaign KPI labels can wrap at 901–1180 without shrinking copy',/campaign-summary small\{font-size:11px;line-height:1\.2;white-space:normal;overflow-wrap:anywhere\}/.test(adminCss));
