...
# DOM contract first: every editorial hero is exactly [heading wrapper, supporting copy].
for route in ROUTES:
    soup=BeautifulSoup((ROOT/route/'index.html').read_text(encoding='utf-8',errors='ignore'),'html.parser')
    hero=soup.select_one('section.inner-hero')
    check(hero is not None,f'/{route}/ has an inner hero')
    if not hero: continue
    kids=[x for x in hero.children if getattr(x,'name',None)]
    check(len(kids)==2 and kids[0].name=='div' and kids[1].name=='p',f'/{route}/ hero uses [heading wrapper + supporting copy] grid contract')
    check(kids[0].find('h1') is not None and kids[0].find(class_='eyebrow') is not None,f'/{route}/ keeps eyebrow and H1 in the left grid cell')
    ...
    try:
        for width,height in [(1650,960),(1280,800),(1024,768),(430,932),(390,844)]:
            for route in ROUTES:
                ...
                metrics=page.evaluate('''()=>{const hero=document.querySelector('.inner-hero'),h1=hero?.querySelector('h1'),copy=[...hero.children].find(x=>x.tagName==='P');const rr=x=>{const r=x?.getBoundingClientRect();return r?{x:r.x,y:r.y,w:r.width,h:r.height,right:r.right,bottom:r.bottom}:null};return{docW:document.documentElement.scrollWidth,viewW:innerWidth,hero:rr(hero),h1:rr(h1),copy:rr(copy)}}''')
