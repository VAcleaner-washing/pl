...
with sync_playwright() as p:
 ...
 for w in (320,390,430):
  ...
  primary=page.locator('#clientEditor .client-primary-actions')
  contacts=page.locator('#clientEditor .client-contact-actions')
  check(actions.count()==1 and primary.count()==1 and contacts.count()==1,f'{w}: client quick actions are grouped by job instead of one flat button stack')
  visible_children=page.locator('#clientEditor .client-primary-actions > *:visible, #clientEditor .client-contact-actions > *:visible, #clientEditor .client-referral-quick:visible')
  check(visible_children.count()>=4,f'{w}: all available client actions remain visible')
  ...
  check(page.locator('#clientEditor .client-referral-quick.primary').count()==0,f'{w}: referral stays visible without competing with New rental as primary CTA')
 ...
 for w,h in ((1648,927),(1440,900),(1280,800),(1024,768)):
  ...
  child_boxes=page.locator('#clientEditor .client-primary-actions > *:visible, #clientEditor .client-contact-actions > *:visible, #clientEditor .client-referral-quick:visible').evaluate_all('els=>els.map(el=>el.getBoundingClientRect())')
