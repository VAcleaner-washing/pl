...
def overflow(page):
 return page.evaluate("""()=>({sw:document.documentElement.scrollWidth,cw:document.documentElement.clientWidth,bad:[...document.querySelectorAll('body *')].filter(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return s.position!=='fixed'&&!el.classList.contains('final-cta-orbit')&&r.width>1&&(r.right>innerWidth+2||r.left<-2)}).slice(0,12).map(el=>String(el.className||el.tagName))})""")
