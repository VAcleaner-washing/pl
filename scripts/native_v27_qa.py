...
def boot(browser,width=390,height=844,add_cancelled=False):
    ...
    p.evaluate("html=>{document.body.innerHTML=html;document.documentElement.classList.add('glass-test','native-test','native-v2','native-v21','native-v22','native-v23','native-v24','native-v25','native-v26','native-v27')}",INITIAL)
...
with sync_playwright() as pw:
    ...
    # V2.6 screenshot-derived visual contracts.
    p=boot(b);open_view(p,'bookings')
    toolbar=p.locator('.booking-toolbar')
    check(toolbar.count()==1 and toolbar.evaluate("e=>getComputedStyle(e).position==='static'"),'V2.6: booking filters scroll with content, no card overlap')
    chips=p.locator('.booking-toolbar .chip:visible')
    check(all(x>=44 for x in chips.evaluate_all("els=>els.map(e=>e.getBoundingClientRect().height)")),'V2.6: booking filter targets 44px+')
    p.locator('[data-filter="completed"]').click();p.wait_for_timeout(60)
    compact=p.locator('.booking-card.native-v22-compact').first
    if compact.count():
        check(compact.locator('.booking-finance>em.neutral:visible').count()==0,'V2.6: zero final balance does not waste compact-card row')
    p.close()
    ...
    if entry.count():
        check(entry.evaluate("e=>getComputedStyle(e).borderRadius==='0px'"),'V2.6: audit history rows are flat')
    reload=p.locator('#auditReload:visible')
    if reload.count():check(reload.evaluate("e=>e.getBoundingClientRect().height>=44"),'V2.6: audit reload 44px touch target')
    ...
    if more.count():
        ...
        check(actions.count()>=1 and all(x>=50 for x in actions.evaluate_all("els=>els.map(e=>e.getBoundingClientRect().height)")),'V2.6: action sheet uses full-height list rows')
    p.close()

    p=boot(b);p.locator('.mobile-nav .more-nav').click();p.wait_for_selector('.mobile-more-menu');p.wait_for_timeout(50)
    check(p.locator('.native-v23-connection:visible').count()==0,'V2.6: standalone connection row removed')
    meta=p.locator('.native-profile-card span>small:visible')
    check(meta.count()==1 and 'Адміністратор' in meta.inner_text(),'V2.6: profile metadata retained')
    p.close()
    ...
    for action,formsel in [('issue','.issue-form'),('finance','.finance-form')]:
        ...
        if loc.count():
            ...
            check(head.count()==1 and head.evaluate("e=>parseFloat(getComputedStyle(e).borderTopWidth)<=1"),f'V2.6: {action} context has one thin shell')
        p.close()

    p=boot(b);open_view(p,'campaigns')
    sm=p.locator('.campaign-metrics small:visible')
    if sm.count():check(min(sm.evaluate_all("els=>els.map(e=>parseFloat(getComputedStyle(e).fontSize))"))>=10,'V2.6: campaign metric labels readable')
    p.close()

    p=boot(b);open_view(p,'calendar')
    labels=p.locator('.day-labels>span:visible')
    if labels.count():check(min(labels.evaluate_all("els=>els.map(e=>parseFloat(getComputedStyle(e).fontSize))"))>=10.5,'V2.6: calendar column labels readable')
    p.close()
    # V2.6 user-reported regressions: clipping, upcoming rail, More profile and process documents.
    for width in (320,390,430):
        p=boot(b,width);open_view(p,'analytics')
        period=p.locator('.analytics-periods .chip:visible, .admin-periods .chip:visible')
        check(all(x['left']>=-1 and x['right']<=width+1 for x in period.evaluate_all("els=>els.map(e=>{let r=e.getBoundingClientRect();return {left:r.left,right:r.right}})")),f'V2.6 {width}px: analytics periods fully inside viewport')
        open_view(p,'finances');period=p.locator('.analytics-periods .chip:visible, .admin-periods .chip:visible')
        check(all(x['left']>=-1 and x['right']<=width+1 for x in period.evaluate_all("els=>els.map(e=>{let r=e.getBoundingClientRect();return {left:r.left,right:r.right}})")),f'V2.6 {width}px: finance periods fully inside viewport')
        open_view(p,'clients');sels=p.locator('.clients-toolbar select:visible')
        check(all(x['left']>=-1 and x['right']<=width+1 for x in sels.evaluate_all("els=>els.map(e=>{let r=e.getBoundingClientRect();return {left:r.left,right:r.right}})")),f'V2.6 {width}px: clients segment/sort controls fully visible')
        open_view(p,'upcoming')
        check(p.locator('.upcoming-row .upcoming-time i:visible').count()>0 and p.locator('.upcoming-row .schedule-badge:visible').count()>0,f'V2.6 {width}px: upcoming restores arrow + relative day badge')
        p.close()
    ...
    if fuel.count():
        ...
        check(one_shell,'V2.6: fuel numeric controls have one visible shell')
    tabs=p.locator('.settings-tabs .settings-tab:visible')
    check(tabs.count()==5,'V2.6: all five settings tabs visible without clipped rail')
    p.locator('.mobile-nav .more-nav').click();p.wait_for_timeout(40)
    check(p.locator('.native-profile-card.native-v25-profile-static').count()==1,'V2.6: More profile is informational, not Settings shortcut')
    p.close()
    ...
    if proc.count():
        ...
        check(p.locator('[data-v25-document-file]').count()==1,'V2.6: process exposes document photo upload')
        check(p.locator('[data-v25-document-view]').count()==1,'V2.6: process exposes document photo viewer')
    p.close()
    ...
    if camp.count():
        ...
        title=p.locator('.sms-heading h2').inner_text();check('RETURN' in title,'V2.6: RETURN campaign title remains readable')
        ...
        check(geom['bt']>=geom['hb']-1 and geom['sw']<=geom['cw']+1,'V2.6: RETURN SMS header/body do not overlap or overflow')
    p.close()

    b.close()

# Static PWA isolation/deep-link contracts.
manifest=json.loads((ROOT/'admin/manifest-native-v27.webmanifest').read_text())
check(manifest['name']=='VAcleaner Native V2.7 Full Re-audit RC','Manifest: V2.7 name')
check(manifest['start_url']=='/admin/bronuvannia-native-v27/' and manifest['scope']=='/admin/bronuvannia-native-v27/','Manifest: V2.7 route/scope')
sw=(ROOT/'admin/sw-native-v27.js').read_text()
check("const FALLBACK=ROUTE" in sw,'SW: offline fallback stays on V2.7')
check("/^\\/admin\\/bronuvannia\\/?$/" in sw and "target.pathname=ROUTE" in sw,'SW: production deep links rewrite to V2.7')
route=(ROOT/'admin/bronuvannia-native-v26/index.html').read_text()
check('/assets/admin-native-v25.css?v=4247' in route and '/assets/admin-native-v25.js?v=4247' in route,'Route: V2.6 assets loaded')
v25js=(ROOT/'assets/admin-native-v25.js').read_text()
check("const ROUTE='/admin/bronuvannia-native-v27/'" in v25js and 'data:{url:ROUTE}' in v25js and '__nativeV26=true' in v25js,'Local notifications: V2.6 route wins over inherited V2.3 patch')

print(f'NATIVE V2.7 QA: PASS {passed} · FAIL {len(failed)}')
if failed:
