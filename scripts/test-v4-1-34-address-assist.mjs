...
ok('release 4.1.34',release.version==='4.1.34'&&Number(release.build)===4134);
ok('public loads address helper',pub.includes('/assets/address-autocomplete.js?v=4134')&&pub.includes('/assets/address-autocomplete.css?v=4134'));
ok('admin loads address helper',admin.includes('/assets/address-autocomplete.js?v=4134')&&admin.includes('/assets/address-autocomplete.css?v=4134'));
ok('autocomplete endpoint is Supabase proxy',runtime.includes('vacleaner-address-v1')&&runtime.includes('Почніть вводити вулицю'));
...
ok('delivery details are separate',runtime.includes('Квартира / під’їзд / поверх')&&runtime.includes('DETAILS_SEPARATOR'));
...
ok('edge function scopes lookup to Poltava',edge.includes('POLTAVA_BBOX')&&edge.includes('countrycode')&&edge.includes('Полтава'));
ok('edge function uses OSM Photon with timeout',edge.includes('photon.komoot.io')&&edge.includes('3500'));
