...
ok('admin customer lookup restores clean address and legacy access detail into the separate address-detail control',adminJs.includes("savedDeliveryValue=[savedDelivery.address,savedDelivery.note].filter(Boolean).join(' · ')")&&adminJs.includes('__VAC_SET_ADMIN_DELIVERY_ADDRESS__?.(savedDeliveryValue)')&&!adminJs.includes('form.customerComment.value=savedDelivery.note'));
