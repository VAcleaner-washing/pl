...
ok('public create preserves legacy UI integration and sends canonical parts',bookingChunk.includes('globalThis.__VAC_DELIVERY_ADDRESS__?.(S)')&&bookingSlots.includes('body.deliveryAddressDetail=addressParts.detail'));
ok('admin create/edit submits canonical address parts',adminJs.includes("window.__VAC_ADMIN_DELIVERY_ADDRESS_PARTS__?.(fd.get('deliveryAddress'))")&&adminJs.includes('deliveryAddressDetail:deliveryParts.detail'));
ok('navigator ignores entrance/orientation details',adminJs.includes('function deliveryAddressParts(value)')&&adminJs.includes('const address=deliveryAddressParts(value).address')&&adminJs.includes('navigationDestination(value)'));
ok('admin customer lookup restores clean address and access detail into the separate control',adminJs.includes('savedDeliveryDetail=customer.addressDetail||savedDelivery.note')&&adminJs.includes('__VAC_SET_ADMIN_DELIVERY_ADDRESS__?.(savedDeliveryAddress,savedDeliveryDetail)')&&!adminJs.includes('form.customerComment.value=savedDelivery.note'));
