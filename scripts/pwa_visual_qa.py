...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        section_geometry=page.locator('.sms-campaign-modal .sms-history-summary').evaluate("""el=>{const k=el.querySelector('small').getBoundingClientRect(),h=el.querySelector('h3').getBoundingClientRect();return{k:{r:k.right,b:k.bottom},h:{l:h.left,t:h.top},scroll:getComputedStyle(el.closest('.sms-form-body')).scrollbarWidth}}""")
        ...
        qa.check(not page.locator('.sms-campaign-modal .sms-history').evaluate('el=>el.open'), f"{label}: secondary SMS journal stays collapsed by default")
        ...
        mobile_audience_scroll=page.locator('.sms-campaign-modal #smsAudienceList').evaluate("""el=>{const h=el.clientHeight,overflow=getComputedStyle(el).overflowY;const sample=el.querySelector('.sms-recipient');const sampleH=sample?sample.getBoundingClientRect().height:0;if(sample){for(let i=0;i<12;i++)el.append(sample.cloneNode(true));}el.scrollTop=140;return{h,overflow,scrollTop:el.scrollTop,scrollHeight:el.scrollHeight,sampleH,capacity:sampleH?h/sampleH:0}}""")
        min_mobile_capacity=5.5 if width<=360 else 6.5
        qa.check(mobile_audience_scroll['h']>=440, f"{label}: SMS recipient viewport gives the client list most of the mobile workspace")
        qa.check(mobile_audience_scroll['capacity']>=min_mobile_capacity, f"{label}: SMS recipient viewport shows roughly 7 clients before list scrolling")
        qa.check(mobile_audience_scroll['overflow'] in ('auto','scroll') and mobile_audience_scroll['scrollTop']>0, f"{label}: SMS client list owns a clear internal scroll")
    ...
    try:
        ...
        for view in ["bookings", "calendar", "upcoming", "equipment", "clients", "campaigns", "analytics", "chemistry", "settings"]:
            ...
            if view == "campaigns":
                ...
                qa.check(audience_scroll['h']>=260, "Desktop: SMS recipient viewport is large enough for practical selection")
                ...
                qa.check(not page.locator('.sms-campaign-modal .sms-history').evaluate('el=>el.open'), "Desktop: secondary SMS journal stays collapsed until requested")
                desktop_modal=page.locator('.sms-campaign-modal').evaluate("""el=>{const r=el.getBoundingClientRect(),header=el.querySelector(':scope>header').getBoundingClientRect(),footer=el.querySelector(':scope>footer').getBoundingClientRect(),actions=el.querySelector('.sms-audience-actions').getBoundingClientRect(),send=el.querySelector('#smsSend').getBoundingClientRect();return{w:r.width,h:r.height,header:header.height,footer:footer.height,r:{l:r.left,r:r.right},actions:{l:actions.left,r:actions.right},send:{l:send.left,r:send.right}}}""")
                qa.check(desktop_modal['w']>=1000 and desktop_modal['header']<=130 and desktop_modal['footer']<=80, "Desktop: SMS modal keeps a wide working area with compact chrome")
                qa.check(desktop_modal['actions']['l']>=desktop_modal['r']['l']-1 and desktop_modal['actions']['r']<=desktop_modal['r']['r']+1, "Desktop: SMS sorting and bulk-selection controls stay inside the modal")
                qa.check(desktop_modal['send']['l']>=desktop_modal['r']['l']-1 and desktop_modal['send']['r']<=desktop_modal['r']['r']+1, "Desktop: SMS primary footer action is never clipped by the modal edge")
