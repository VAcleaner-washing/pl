...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        page.wait_for_timeout(80)
        qa.check(page.locator('.mobile-nav:visible').count()==1 and abs(page.locator('.mobile-nav').bounding_box()['y']+page.locator('.mobile-nav').bounding_box()['height']-height)<=1, f"{label}: bottom nav returns to exact bottom after keyboard closes")
