...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        mobile_audience_scroll=page.locator('.sms-campaign-modal .sms-recipient-step').evaluate("""el=>{const list=el.querySelector('#smsAudienceList'),toolbar=el.querySelector('.sms-recipient-toolbar'),sample=list?.querySelector('.sms-recipient');const before=toolbar?.getBoundingClientRect().top||0;if(sample&&list){for(let i=0;i<12;i++){const c=sample.cloneNode(true);c.classList.add('qa-clone');list.append(c);}}el.scrollTop=140;const after=toolbar?.getBoundingClientRect().top||0;const out={overflow:getComputedStyle(el).overflowY,scrollTop:el.scrollTop,scrollHeight:el.scrollHeight,clientHeight:el.clientHeight,toolbarMoved:before-after,listOverflow:list?getComputedStyle(list).overflowY:'',listScrollTop:list?.scrollTop||0};list?.querySelectorAll('.qa-clone').forEach(x=>x.remove());el.scrollTop=0;return out}""")
        qa.check(mobile_audience_scroll['overflow'] in ('auto','scroll') and mobile_audience_scroll['scrollTop']>0 and mobile_audience_scroll['scrollHeight']>mobile_audience_scroll['clientHeight'], f"{label}: SMS recipient step is the single mobile scroll owner")
        qa.check(mobile_audience_scroll['toolbarMoved']>40 and mobile_audience_scroll['listOverflow']=='visible' and mobile_audience_scroll['listScrollTop']==0, f"{label}: audience controls and recipient list scroll together without nested list scrolling")
