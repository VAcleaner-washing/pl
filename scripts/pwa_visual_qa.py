...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        qa.check(shell_position=='fixed' and nav_position=='fixed', f"{label}: installed PWA bottom navigation is fixed directly to the viewport")
        ...
        qa.check(page.locator('.nav button[data-view="analytics"]:visible').count()==0 and page.locator('#mobileNewBooking:visible').count()==1, f"{label}: center New replaces analytics in the five primary bottom-nav actions")
        new_box=page.locator('#mobileNewBooking:visible').bounding_box(); regular_box=page.locator('.nav button[data-view="calendar"]:visible').bounding_box()
        ...
        qa.check(page.locator('.more-nav.active:visible').count()==1 and page.locator('.nav button[data-view="analytics"]:visible').count()==0, f"{label}: analytics activates More without a second visible active item")
        ...
        page.wait_for_timeout(40)
        keyboard_nav_state=page.locator('.sidebar').evaluate("el=>({display:getComputedStyle(el).display,visibility:getComputedStyle(el).visibility,opacity:getComputedStyle(el).opacity,position:getComputedStyle(el).position,bottom:getComputedStyle(el).bottom})")
        qa.check(page.locator('.sidebar:visible').count()==1 and keyboard_nav_state['display']!='none' and keyboard_nav_state['visibility']=='visible' and keyboard_nav_state['position']=='fixed' and keyboard_nav_state['bottom']=='0px', f"{label}: keyboard does not mutate or recreate fixed bottom nav (VA HOME contract)")
