...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        mobile_audience_scroll=page.locator('.sms-campaign-modal #smsAudienceList').evaluate("""el=>{const h=el.clientHeight,overflow=getComputedStyle(el).overflowY;const sample=el.querySelector('.sms-recipient');if(sample){for(let i=0;i<12;i++)el.append(sample.cloneNode(true));}el.scrollTop=140;return{h,overflow,scrollTop:el.scrollTop,scrollHeight:el.scrollHeight}}""")
        qa.check(mobile_audience_scroll['h']>=230, f"{label}: SMS recipient viewport prioritizes the client list")
    ...
    try:
        ...
        for view in ["bookings", "calendar", "upcoming", "equipment", "clients", "campaigns", "analytics", "chemistry", "settings"]:
            ...
            if view == "campaigns":
                ...
                desktop_modal=page.locator('.sms-campaign-modal').evaluate("""el=>{const r=el.getBoundingClientRect(),header=el.querySelector(':scope>header').getBoundingClientRect(),footer=el.querySelector(':scope>footer').getBoundingClientRect();return{w:r.width,h:r.height,header:header.height,footer:footer.height}}""")
                qa.check(desktop_modal['w']>=900 and desktop_modal['header']<=130 and desktop_modal['footer']<=80, "Desktop: SMS modal keeps a wide working area with compact chrome")
