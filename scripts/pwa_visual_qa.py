...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        qa.check(page.locator('.sidebar:visible').count()==0 and keyboard_nav_state['display']!='none' and keyboard_nav_state['visibility']=='hidden' and keyboard_nav_state['position']=='fixed' and keyboard_nav_state['bottom']=='0px', f"{label}: keyboard hides bottom nav without destroying its fixed layer")
