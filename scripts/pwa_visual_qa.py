...
def mobile_suite(browser: Browser, qa: QA, width: int, label: str) -> None:
    ...
    try:
        ...
        qa.check(shell_position=='fixed' and nav_position=='absolute', f"{label}: fixed viewport shell owns an absolute bottom navigation")
        ...
        qa.check(page.locator(".detail-top").evaluate("el=>getComputedStyle(el).position") != "sticky", f"{label}: detail header scrolls naturally and never covers finance/history content")
