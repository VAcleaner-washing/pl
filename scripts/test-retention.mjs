...
check('SMS promo discovery resolves only promos actually sent to that customer phone',()=>{assert.ok(phonePromo.includes('vacleaner_sms_dispatch_recipients'));assert.ok(phonePromo.includes('["submitted","sent","delivered"]'));assert.ok(phonePromo.includes('customer_phone'));assert.ok(phonePromo.includes('source:"sms_phone"'))});
...
check('campaign card exposes SMS denominator when conversion is SMS-based',()=>{assert.ok(admin.includes("c.conversionBasis==='sms'"));assert.ok(admin.includes('`${smsCount} SMS · ${conversionUses} використано`'))});
check('public booking auto-fills a discovered SMS promo without overwriting a manual or link promo',()=>{assert.ok(publicBooking.includes('PHONE_PROMO_API'));assert.ok(publicBooking.includes('applySmsPromoCode'));assert.ok(publicBooking.includes('promo.dataset.autoSmsPromo'));assert.ok(publicBooking.includes('Персональний бонус із SMS'));assert.ok(publicBooking.includes('промокод уже підставлено в бронювання'))});
