...
check('public promo never stacks with loyalty',()=>{assert.ok(booking.includes('const loyaltyReferralBundle = loyaltyDiscount + referralCandidateDiscount'));assert.ok(booking.includes('const promoApplied = Boolean'));assert.ok(booking.includes('baseAmount = Math.max(0, rawBase - discount)'));assert.ok(!booking.includes('loyaltyDiscount + promoDiscount'))});
check('SMS promo discovery resolves only promos actually sent to that customer phone',()=>{assert.ok(phonePromo.includes('vacleaner_sms_dispatch_recipients'));assert.ok(phonePromo.includes('["submitted","sent","delivered"]'));assert.ok(phonePromo.includes('customer_phone'));assert.ok(phonePromo.includes('qualifyingDispatch'));assert.ok(phonePromo.includes('.eq("active",true)'))});
check('campaign conversion uses successful SMS recipients instead of generated RETURN codes',()=>{assert.ok(data.includes('vacleaner_sms_dispatches'));assert.ok(data.includes('vacleaner_sms_dispatch_recipients'));assert.ok(data.includes('smsRecipientCount:smsPhones.size'));assert.ok(data.includes('conversionBasis:hasSmsDispatch?"sms":"codes"'));assert.ok(data.includes('conversionBase=hasSmsDispatch?smsPhones.size:personalAssigned'));assert.ok(data.includes('smsUsed=used.filter'))});
check('campaign card exposes SMS denominator when conversion is SMS-based',()=>{assert.ok(admin.includes("c.conversionBasis==='sms'"));assert.ok(admin.includes('`${smsCount} SMS · ${Number(c.activatedCount||c.assignedCodes||0)} активовано · ${conversionUses} використано`'))});
check('public booking auto-fills a discovered SMS promo without overwriting a manual or link promo',()=>{assert.ok(publicBooking.includes('PHONE_PROMO_API'));assert.ok(publicBooking.includes('applySmsPromoCode'));assert.ok(publicBooking.includes('promo.dataset.autoSmsPromo'));assert.ok(publicBooking.includes('Активований персональний бонус'));assert.ok(publicBooking.includes('Бонус уже активований за вашим номером'))});
