...
check('public promo never stacks with loyalty',()=>{assert.ok(booking.includes('promoDiscount > loyaltyDiscount'));assert.ok(booking.includes('baseAmount = Math.max(0, rawBase - discount)'));assert.ok(!booking.includes('loyaltyDiscount + promoDiscount'))});
