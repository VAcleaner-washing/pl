...
const home=read('index.html'),booking=read('bronuvannia/index.html'),pidbir=read('pidbir/index.html'),e2e=read('scripts/e2e_smoke.py'),pwaQa=read('scripts/pwa_visual_qa.py'),workflow=read('.github/workflows/pages.yml'),qaRunner=read('scripts/qa-full.mjs'),quiz=read('assets/public-quiz.js');
...
const semverAtLeast=(value,minimum)=>{const a=String(value).split('.').map(Number),b=String(minimum).split('.').map(Number);for(let i=0;i<3;i++){if((a[i]||0)>(b[i]||0))return true;if((a[i]||0)<(b[i]||0))return false}return true};
ok('release keeps v4.1.49+ performance contract',semverAtLeast(rel.version,'4.1.49')&&Number(rel.build)>=4149&&pkg.version===rel.version);
...
ok('browser suites use one canonical runner that aggregates all results',workflow.includes('npm run qa:browser')&&qaRunner.includes('for(const s of browserSuites) npm(s)')&&qaRunner.includes('qa-release-summary.json'));
ok('deploy waits for both validation and browser QA',workflow.includes('needs: [validate, browser]'));
ok('v4.1.48 content tests are part of canonical CI',qaRunner.includes('test:v4.1.48-content-local-demand')&&qaRunner.includes('test:v4.1.48-content-visual'));
ok('v4.1.49 performance contract is part of canonical CI',qaRunner.includes('test:v4.1.49-performance-ci'));
