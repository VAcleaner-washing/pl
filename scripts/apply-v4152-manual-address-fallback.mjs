...
// Keep the manual-address fallback explicit after older release normalizers run.
{
  ...
  if(!js.includes("quote.zone==='manual'")||!js.includes('const fallbackTariffs='))throw new Error('Manual address fallback runtime is missing');
  write(rel,js);
}

{
  ...
  if(!js.includes('Точного збігу немає.')&&!js.includes('Введіть адресу вручну'))throw new Error('Manual address status copy is missing');
  write(rel,js);
}
...
for(const rel of ['bronuvannia/index.html','bronuvannia/index.txt','_next/static/chunks/146ntlcv_t6~w-v4041.js']){
  ...
  value=value.split('до вас і назад · 250 грн').join('до вас і назад · Полтава 250 грн');
  write(rel,value);
}
