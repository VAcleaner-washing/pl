...
import { DEFAULT_CATALOG, DEFAULT_DELIVERY_FEE, DEFAULT_DELIVERY_PRICING, DEFAULT_DEPOSIT_RULES, DEFAULT_SLOTS, rentalDays, rentalBase, paidDayMoments, isWeekendDeposit, slotIndex } from "./config.deploy.js";
...
function inWindow(time, window, slots) {
    return window === "morning" ? time >= slots.morningStart && time <= slots.morningEnd : time >= slots.eveningStart && time <= slots.eveningEnd;
}
function depositGroup(productCode, catalog = defaultCatalog) {
    return catalog?.products?.[productCode]?.depositGroup || defaultCatalog.products?.[productCode]?.depositGroup || "oneUnit";
}
function calculateDeposit(productCode, startDate, returnDate, pickupWindow, returnWindow, rules, catalog) {
    const group = depositGroup(productCode, catalog);
...
function canonicalExtraLookup(code) {
    return code === "premium_sc2" ? "premium_nozzles" : code;
}
function normalizeSelectedExtras(value, productCode, catalog) {
...
async function availability(supabase, product, period, excludeId = "") {
    ...
    if (!Number.isFinite(requestStart) || !Number.isFinite(requestEnd) || requestEnd <= requestStart) return {
        available: false,
        remaining: {}
    };
    const [{ data: inventory, error: inventoryError }, { data: bookings, error: bookingsError }] = await Promise.all([
        ...
        supabase.from("vacleaner_bookings").select("id,status,hold_expires_at,start_date,return_date,pickup_window,return_window,vacleaner_booking_resources(resource_code,quantity)").in("status", [
            "waiting_payment",
            "confirmed",
            "issued"
        ])
    ...
    const active = (bookings || []).filter((row)=>row.id !== excludeId && isActiveBooking(row));
    const remaining = {};
    let available = true;
...
function resourcePayload(product) {
    return Object.entries(product?.resources || {}).map(([resource_code, quantity])=>({
            resource_code,
            quantity: Number(quantity || 0)
        })).filter((row)=>row.quantity > 0);
}
async function applyReservation(supabase, bookingId, period, product, targetStatus, holdExpiresAt = null) {
...
async function tagAudit(supabase, bookingId, actorId, source, since) {
    if (!validBookingId(bookingId)) return;
    const { error } = await supabase.from("vacleaner_booking_audit").update({
        actor_id: actorId,
        source
...
async function upsertCustomer(supabase, body, fallback = {}) {
    ...
    const customer = {
        ...
        preferred_contact: [
            ...
            "telegram",
            "instagram"
        ].includes(String(body.preferredContact ?? fallback.preferred_contact ?? "")) ? String(body.preferredContact ?? fallback.preferred_contact) : "phone",
    ...
    if (documentNumber) {
        ...
        ].includes(documentType) ? documentType : "Паспорт";
    ...
    const { error } = await supabase.from("vacleaner_customers").upsert(customer, {
        onConflict: "phone"
    });
    if (error) throw error;
}
async function referralSummaryForPhone(supabase, phoneValue) {
...
Deno.serve(async (request)=>{
    ...
    try {
        ...
        if (!supabaseUrl || !serviceRoleKey || !token) return json({
            error: "unauthorized"
        }, 401);
        ...
        const { data: settingsRows } = await supabase.from("vacleaner_settings").select("key,value").in("key", [
            "deposit_rules",
            "catalog",
            "booking_slots",
            "delivery_fee"
        ]);
        const settingsMap = Object.fromEntries((settingsRows || []).map((row)=>[
                row.key,
                row.value
            ]));
        ...
        if (action === "list") {
            const { data, error } = await supabase.from("vacleaner_bookings").select("*,vacleaner_booking_resources(resource_code,quantity)").order("start_at", {
                ascending: false
            }).limit(1000);
            if (error) throw error;
            return json({
                bookings: (data || []).map((row)=>safeBooking(row))
            });
        }
        if (action === "calendar") {
            ...
            const active = (bookings || []).filter((row)=>isActiveBooking(row));
            const base = Date.parse(`${from}T12:00:00Z`);
            const days = Array.from({
                length: requestedDays
            }, (_, index)=>{
                const date = new Date(base + index * 86400000).toISOString().slice(0, 10), resources = {};
                for (const item of inventory || []){
                    const capacity = Number(item.capacity || 0);
                    const countAt = (window)=>active.reduce((sum, booking)=>{
                            ...
                            const row = booking.vacleaner_booking_resources?.find((r)=>r.resource_code === item.resource_code);
        ...
        if (action === "save_customer") {
            ...
            if (!originalPhone || !customerPhone || customerName.length < 2) return json({
            ...
            const documentType = documentNumber && [
                ...
                "ID-картка",
                "Водійське посвідчення"
            ].includes(requestedType) ? requestedType : documentNumber ? "Паспорт" : null;
            ...
            if (existing) {
                const { error } = await supabase.from("vacleaner_customers").update(row).eq("phone", originalPhone);
                if (error) throw error;
            } else {
                const { error } = await supabase.from("vacleaner_customers").upsert({
                    ...row,
                    created_at: now
                }, {
                    onConflict: "phone"
            ...
            const bookingPatch = {
                ...
                customer_telegram: row.telegram,
        ...
        if (action === "lookup_customer") {
            ...
            const promo = await resolvePhonePromo(supabase, {
                ...
                history: orders || [],
                includeBlocked: true,
                excludeBookingId: body.bookingId
            });
            ...
            const completedOrders = completed.length, loyalty = completedOrders >= 6 ? {
                level: "VIP",
                percent: 10
            } : completedOrders >= 3 ? {
                level: "Regular",
            ...
            return json({
                customer: {
                    phone,
                    name: profile?.name || latest?.customer_name || "",
                    telegram: profile?.telegram || latest?.customer_telegram || "",
                    ...
                    hasDocumentPhoto: Boolean(profile?.document_photo_path),
                    hasDocument,
                    isRepeatCustomer,
                    documentsRequired: !hasDocument && !isRepeatCustomer,
                    completedOrders,
                    totalOrders: (orders || []).filter((row)=>![
                            "cancelled",
                            "declined"
        ...
        if (action === "referrals_expiring") {
            ...
            const { data: rewards, error: rewardError } = await supabase.from("vacleaner_referral_rewards").select("id,referrer_phone,referred_phone,amount,activated_at,expires_at,reminded_at").eq("status", "active").is("reminded_at", null).gt("expires_at", now.toISOString()).lte("expires_at", until).order("expires_at", {
                ascending: true
            }).limit(200);
            if (rewardError) throw rewardError;
            const phones = [
                ...new Set((rewards || []).map((row)=>normalizePhone(row.referrer_phone)).filter(Boolean))
            ];
            ...
            if (phones.length) {
                ...
                customerMap = new Map((customers || []).map((row)=>[
                        normalizePhone(row.phone),
                        row
                    ]));
            }
            return json({
                rewards: (rewards || []).map((row)=>({
                        ...row,
                        customer: customerMap.get(normalizePhone(row.referrer_phone)) || null
                    }))
            });
        }
        if (action === "referral_mark_sent") {
            const phone = normalizePhone(body.phone), channel = [
                "telegram",
                "instagram"
            ].includes(String(body.channel || "")) ? String(body.channel) : "";
            ...
            const applyConfirmedState = async (sentAt)=>{
                const customerPatch = {
                    updated_at: sentAt
                };
                if (!isReminder) {
                    customerPatch.referral_sent_at = sentAt;
        ...
        if (action === "detach_promo") {
            ...
            if (!validBookingId(bookingId)) return json({
            ...
            const extras = {
                ...
                discount: {
                    ...
                    reason: discount.source === "manual" ? discount.manualReason : ""
                },
            ...
            const { error: detachError } = await supabase.rpc("vacleaner_admin_detach_booking_promo", {
                p_booking_id: bookingId,
                p_extras: extras,
            ...
            const { data: saved, error: savedError } = await supabase.from("vacleaner_bookings").select("*").eq("id", bookingId).single();
            if (savedError || !saved) throw savedError || new Error("update_failed");
        ...
        if (action === "attach_promo") {
            ...
            const currentExtras = booking.extras && typeof booking.extras === "object" ? booking.extras : {};
            if (currentExtras?.promo) return json({
                error: "booking_promo_already_attached"
            }, 409);
            const product = catalog.products?.[String(booking.product_code || "")];
            ...
            if (!promo?.promoCodeId || !promo?.campaignId) return json({
                error: "promo_unavailable"
            }, 409);
            const pricingExtras = {
                ...currentExtras,
                promo: promoExtraFromCandidate(promo, true)
            };
            const discount = discountInfo({}, rawBase, pricingExtras);
            if (discount.source !== "promo" || discount.amount <= 0) return json({
                error: "promo_not_best_discount"
            }, 409);
            const extras = {
                ...currentExtras,
                promo: promoExtraFromCandidate(promo, true),
            ...
            const { error: attachError } = await supabase.rpc("vacleaner_admin_attach_booking_promo", {
                ...
                p_customer_phone: normalizePhone(booking.customer_phone),
            ...
            if (attachError) return json({
                error: String(attachError.message || "promo_unavailable").includes("promo_") ? "promo_unavailable" : "service_error"
            }, 409);
            const { data: saved, error: savedError } = await supabase.from("vacleaner_bookings").select("*").eq("id", bookingId).single();
            if (savedError || !saved) throw savedError || new Error("update_failed");
        ...
        if (action === "audit_log") {
            ...
            const { data, error } = await supabase.from("vacleaner_booking_audit").select("id,booking_id,booking_code,event_type,changed_fields,old_values,new_values,actor_id,source,created_at").eq("booking_id", bookingId).order("created_at", {
                ascending: false
            }).limit(limit);
            if (error) throw error;
            return json({
        ...
        if (action === "create" || action === "edit") {
            ...
            if (action === "edit" && !validBookingId(bookingId)) return json({
                error: "invalid_booking"
            }, 400);
            const av = await availability(supabase, product, period, bookingId);
            ...
            let enteredReferral = null;
            if (action === "create" && cleanText(body.referralCode, 32)) {
                const { count: completedCount, error: completedError } = await supabase.from("vacleaner_bookings").select("id", {
                    count: "exact",
                    head: true
                }).eq("customer_phone", customerPhone).eq("status", "completed");
                if (completedError) throw completedError;
                enteredReferral = await validateFriendReferral(supabase, body.referralCode, customerPhone, completedCount || 0);
                if (!enteredReferral?.valid) return json({
                    error: enteredReferral?.reason || "invalid_referral",
            ...
            const pricingBody = rewardCandidate ? {
                ...body,
                referralRewardAmount: Number(rewardCandidate.amount || 150)
            } : body;
            ...
            const processing = requestedProcessing ? {
                ...
                confirmation_sent: requestedProcessing.confirmation_sent === true,
                documents_required: requestedProcessing.documents_required === true,
                identity_verified: requestedProcessing.identity_verified === true,
                customer_kind: [
                    "new",
                    "repeat",
                    "known"
                ].includes(String(requestedProcessing.customer_kind || "")) ? String(requestedProcessing.customer_kind) : "known",
                updated_at: new Date().toISOString()
            } : currentExtras?.processing;
            const selectedItems = selected.items;
            const selectedAmount = selected.amount;
            const extras = {
                ...currentExtras,
                pickup_time: period.pickupTime,
                return_time: period.returnTime,
                slot_config: slots,
                ...processing ? {
                    processing
                } : {},
                discount: {
                    ...
                    amount: discount.amount,
                    source: discount.source,
                    primary_source: discount.primarySource,
                    loyalty_amount: discount.loyaltyAmount,
                    referral_reward_amount: discount.referralRewardAmount,
                    reason: discount.primarySource === "manual" ? discount.manualReason : ""
                },
                manual_discount: discount.manualType === "none" ? null : {
                    type: discount.manualType,
                    value: discount.manualValue,
                    amount: discount.manualAmount,
                    reason: discount.manualReason
                },
                loyalty: {
                    level: String(body.loyaltyLevel || currentExtras?.loyalty?.level || (discount.loyaltyPercent === 10 ? "VIP" : discount.loyaltyPercent === 5 ? "Regular" : "Start")),
                ...
                promo: promoApplied ? {
                    ...promoExtraFromCandidate(autoPromo, true),
                    kind: autoPromo?.kind || "campaign",
                    referrer_phone: autoPromo?.referrerPhone || null
                } : currentExtras?.promo || null,
                referral_friend: enteredReferral?.valid ? {
                    code: enteredReferral.code,
                    referrer_phone: enteredReferral.referrerPhone,
                    discount_amount: 100
                } : currentExtras?.referral_friend || null,
                referral_reward: discount.referralRewardAmount > 0 && rewardCandidate ? {
                    id: rewardCandidate.id,
                    amount: discount.referralRewardAmount,
                    expires_at: rewardCandidate.expires_at,
                    applied: true
                } : currentExtras?.referral_reward || null,
            ...
            const common = {
                ...
                start_at: `${period.startDate}T${period.pickupTime}:00.000Z`,
                end_at: `${period.returnDate}T${period.returnTime}:00.000Z`,
                ...
                extras_amount: selectedAmount,
                delivery_amount: deliveryAmount,
                total_amount: discount.baseAmount + selectedAmount + deliveryAmount,
                prepayment_amount: 200,
                prepayment_paid: prepaymentPaid,
                deposit_amount: depositAmount,
                admin_note: cleanAdminNote(body.adminNote, 800) || null,
                updated_at: new Date().toISOString()
            };
            let saved;
            if (action === "create") {
                ...
                if (![
                    ...
                    "waiting_payment",
                    "confirmed"
                ].includes(String(existing.status || ""))) return json({
                    error: "invalid_transition"
                }, 409);
                ...
                const patch = {
                    ...common,
                    status: saved.status,
                    hold_expires_at: saved.hold_expires_at,
                    prepayment_paid: saved.prepayment_paid,
                    prepayment_paid_at: saved.prepayment_paid_at,
                    confirmed_at: saved.confirmed_at
                };
                const { data, error } = await supabase.from("vacleaner_bookings").update(patch).eq("id", bookingId).select("*").single();
            ...
            if (action === "create" && enteredReferral?.valid) {
                try {
                    await registerReferralUse(supabase, {
                        code: enteredReferral.code,
                        referrerPhone: enteredReferral.referrerPhone,
            ...
            if (action === "create" && discount.referralRewardAmount > 0 && rewardCandidate?.id) {
                try {
                    await claimReferralReward(supabase, rewardCandidate.id, saved.id);
                } catch  {
                    await supabase.from("vacleaner_booking_resources").delete().eq("booking_id", saved.id);
                    await supabase.from("vacleaner_bookings").delete().eq("id", saved.id);
                    return json({
        ...
        if (action === "update") {
            const bookingId = String(body.bookingId || ""), nextStatus = String(body.status || "");
            if (!validBookingId(bookingId)) return json({
                error: "invalid_booking"
            }, 400);
            ...
            if (nextStatus === "confirmed" || nextStatus === "waiting_payment") {
                ...
                if (!reservationAllowed.includes(String(current.status || ""))) return json({
                    error: "invalid_transition"
                }, 409);
                const product = catalog.products?.[current.product_code];
                if (!product) return json({
                    error: "invalid_product"
                }, 400);
                const period = {
                    startDate: String(current.start_date),
                    returnDate: String(current.return_date),
                    pickupWindow: current.pickup_window || "morning",
                    returnWindow: current.return_window || "evening",
                    pickupTime: current.extras?.pickup_time || (current.pickup_window === "evening" ? slots.eveningStart : slots.morningStart),
                    returnTime: current.extras?.return_time || (current.return_window === "evening" ? slots.eveningEnd : slots.morningEnd),
                    days: current.rental_days
                };
                ...
                try {
                    ...
                    return json({
                        booking: safeBooking(reserved)
                    });
                } catch (reservationError) {
                    if (reservationError instanceof Error && reservationError.message === "inventory_conflict") return json({
                        error: "inventory_conflict"
                    }, 409);
                    throw reservationError;
                }
            }
            const allowed = nextStatus === "issued" ? [
                "confirmed"
            ...
            if (![
                ...
                "declined",
                "cancelled"
            ].includes(nextStatus) || !allowed.includes(current.status)) return json({
                error: "invalid_transition"
            }, 409);
            const now = new Date().toISOString(), patch = {
                status: nextStatus,
                admin_note: cleanAdminNote(body.adminNote, 800) || cleanAdminNote(current.admin_note, 800) || null,
                updated_at: now
            };
            if (nextStatus === "cancelled") {
                ...
                patch.extras = {
                    ...
                    cancellation: {
                        ...
                        prepayment_refundable: refundable,
                        prepayment_retained: current.prepayment_paid === true && !refundable ? Math.max(0, Number(current.prepayment_amount || 200)) : 0
                    }
                };
            }
            ...
            if (nextStatus === "confirmed") {
                patch.prepayment_paid = true;
                patch.prepayment_amount = 200;
                patch.prepayment_paid_at = current.prepayment_paid_at || now;
            ...
            if ([
                "cancelled",
                "declined"
            ].includes(nextStatus)) await releaseReferralForCancelledBooking(supabase, bookingId);
        ...
        if (action === "save_deposit_return") {
            ...
            if (current.deposit_paid !== true) return json({
            ...
            if (![
                ...
                "issued",
                "completed"
            ].includes(String(current.status || ""))) return json({
                error: "invalid_transition"
            }, 409);
            const finance = settlementFromBooking(current, catalog, defaultCatalog), confirmation = settlementConfirmation(body, finance);
            if (!confirmation.ok) return json({
            ...
            const extras = {
                ...
                settlement: {
                    ...
                    refund_amount: finance.refundAmount,
                    due_amount: finance.dueAmount,
                    refund_paid: finance.refundAmount > 0 ? confirmation.refundPaid : false,
                    due_paid: finance.dueAmount > 0 ? confirmation.duePaid : false,
                    completed: true,
                    completed_at: now,
                    calculated_at: now
                }
            ...
            const { data, error } = await supabase.from("vacleaner_bookings").update({
                ...
                return_payment_paid: finance.dueAmount > 0 ? confirmation.duePaid : false,
                return_payment_paid_at: finance.dueAmount > 0 && confirmation.duePaid ? now : null,
                status: "completed",
        ...
        if (message === "inventory_conflict") return json({
            error: "inventory_conflict"
        }, 409);
        if ([
            "invalid_dates",
