...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const phone = normalizePhone(body.customerPhone); let completed = 0;
    if (phone) {
      ...
      if (error) throw error; completed = count || 0;
    }
    ...
    if (body.action === "availability") return json({ ...av, estimate });
