...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const baseAmount = Math.max(0, rawBase - discount), deliveryAmount = body.fulfillment === "delivery" ? 250 : 0, totalAmount = baseAmount + selected.amount + deliveryAmount;
    ...
    if (customerName.length < 2 || !phone || body.privacyAccepted !== true || (fulfillment === "delivery" && address.length < 8)) return json({ error: "invalid_customer_data" }, 400);
