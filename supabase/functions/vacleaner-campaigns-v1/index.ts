  ...
  try{
    ...
    if(action==="pending_bonus"){
      ...
      for(const promo of promos||[]){
        ...
        const now=Date.now(),starts=campaign.starts_at?new Date(campaign.starts_at).getTime():0,campaignEndsAt=campaign.ends_at?new Date(campaign.ends_at).getTime():0;if(campaign.status!=="active"||(starts&&starts>now)||(campaignEndsAt&&campaignEndsAt<=now))continue;
    ...
    if(action==="activate_bonus"){
      ...
      const starts=campaign.starts_at?new Date(campaign.starts_at).getTime():0,campaignEndsAt=campaign.ends_at?new Date(campaign.ends_at).getTime():0;if(campaign.status!=="active"||(starts&&starts>nowMs)||(campaignEndsAt&&campaignEndsAt<=nowMs))return json({error:"activation_window_expired"},409);
