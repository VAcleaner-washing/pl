...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(action==="sms_audience"){
      const segment=["all","sleeping","sleeping_warm","sleeping_long"].includes(String(body.segment))?String(body.segment):"sleeping";const rows=await buildAudience(db,segment);return json({segment,customers:rows,summary:{total:rows.length,selectable:rows.filter(r=>r.selectable).length,explicit:rows.filter(r=>r.selectable&&r.consent==="explicit").length,legacy:rows.filter(r=>r.selectable&&r.consent==="legacy").length,optedOut:rows.filter(r=>r.consent==="opted_out").length,cooldown:rows.filter(r=>r.cooldown).length,active:rows.filter(r=>r.activeBooking).length}})
    }
    if(action==="sms_dispatches"){
      const {data,error}=await db.from("vacleaner_sms_dispatches").select("id,campaign_id,sender,route,audience_segment,message_body,message_parts,status,sendpulse_campaign_id,audience_count,explicit_consent_count,legacy_count,sent_count,delivered_count,not_delivered_count,legacy_attestation,total_cost,currency,error_code,created_at,sent_at,last_synced_at").order("created_at",{ascending:false}).limit(50);if(error)throw error;return json({dispatches:data||[]})
    }
    ...
    if(action==="sms_send"){
      ...
      const now=new Date().toISOString(),parts=smsParts(message);const {data:dispatch,error:dispatchError}=await db.from("vacleaner_sms_dispatches").insert({campaign_id:validUuid(campaignId)?campaignId:null,sender:SMS_SENDER,route,audience_segment:segment,message_body:message,message_parts:parts,status:"draft",audience_count:recipients.length,explicit_consent_count:explicit.length,legacy_count:legacy.length,legacy_attestation:legacy.length?true:false,created_by:userData.user.id}).select("*").single();if(dispatchError||!dispatch)throw dispatchError||new Error("dispatch_insert_failed");
      const recipientRows=recipients.map(r=>({dispatch_id:dispatch.id,customer_phone:r.phone,customer_name:r.name,consent_basis:r.consent==="explicit"?"explicit":"legacy_admin_attested",status:"queued"}));const {error:recipientError}=await db.from("vacleaner_sms_dispatch_recipients").insert(recipientRows);if(recipientError){await db.from("vacleaner_sms_dispatches").delete().eq("id",dispatch.id);throw recipientError}
      try{
        const result=await spJson(key,"https://api.sendpulse.com/sms/send",{method:"POST",body:JSON.stringify({sender:SMS_SENDER,phones:recipients.map(r=>smsPhone(r.phone)),body:message,route:{UA:route},emulate:false,stat_link_tracking:true,stat_link_need_protocol:true})});if(!result?.result||!result?.campaign_id)throw new Error("sendpulse_api_error");const sends=Math.max(0,Number(result?.counters?.sends||recipients.length)),status=sends===recipients.length?"submitted":"partial";await Promise.all([db.from("vacleaner_sms_dispatches").update({status,sendpulse_campaign_id:Number(result.campaign_id),sent_count:sends,sent_at:now,last_synced_at:now}).eq("id",dispatch.id),db.from("vacleaner_sms_dispatch_recipients").update({status:"submitted",updated_at:now}).eq("dispatch_id",dispatch.id)]);return json({ok:true,dispatchId:dispatch.id,campaignId:Number(result.campaign_id),sent:sends,exceptions:Number(result?.counters?.exceptions||0),parts})
