...
import { DEFAULT_CATALOG, DEFAULT_DEPOSIT_RULES, DEFAULT_SLOTS, rentalDays, rentalBase, isWeekendDeposit, slotIndex, } from "./config.js";
...
const safeBooking = (booking) => { const out = { ...booking }; delete out.ip_hash; return out; };
...
function normalizeSlots(value) {
    ...
    if (!Object.values(out).every(validTime))
        return structuredClone(defaultSlots);
    if (!(out.morningStart < out.morningEnd && out.morningEnd < out.eveningStart && out.eveningStart < out.eveningEnd))
        return structuredClone(defaultSlots);
    return out;
}
...
function canonicalExtraLookup(code) { return code === "premium_sc2" ? "premium_nozzles" : code; }
...
function periodFromBody(body, slots) {
    ...
    if (!validTime(pickupTime) || !validTime(returnTime) || !inWindow(pickupTime, pickupWindow, slots) || !inWindow(returnTime, returnWindow, slots))
        throw new Error("invalid_exact_time");
...
async function availability(supabase, product, period, excludeId = "") {
    ...
    if (inventoryError || bookingsError)
        throw inventoryError ?? bookingsError;
...
async function applyReservation(supabase, bookingId, period, product, targetStatus, holdExpiresAt = null) {
    ...
    if (error) {
        if (String(error.message || "").includes("inventory_conflict"))
...
async function upsertCustomer(supabase, body, fallback = {}) {
    ...
    if (documentNumber) {
        ...
        if (body.identityVerified === true)
            customer.document_verified_at = new Date().toISOString();
    }
...
Deno.serve(async (request) => {
    ...
    try {
        ...
        if (action === "list") {
            ...
            if (error)
                throw error;
            return json({ bookings: (data || []).map((row) => safeBooking(row)) });
        }
        if (action === "calendar") {
        ...
        if (action === "create" || action === "edit") {
            ...
            if (action === "edit" && !validBookingId(bookingId))
                return json({ error: "invalid_booking" }, 400);
            const av = await availability(supabase, product, period, bookingId);
            if (!av.available)
                return json({ error: "inventory_conflict", availability: av }, 409);
            ...
            if (customerName.length < 2 || !customerPhone || (fulfillment === "delivery" && address.length < 8))
            ...
            if (action === "edit" && !existing)
                return json({ error: "invalid_booking" }, 404);
            ...
            const extras = { ...currentExtras, pickup_time: period.pickupTime, return_time: period.returnTime, slot_config: slots,
                discount: { percent: discount.percent, amount: discount.amount, source: discount.source }, loyalty: { level: String(body.loyaltyLevel || currentExtras?.loyalty?.level || (discount.loyaltyPercent === 10 ? "VIP" : discount.loyaltyPercent === 5 ? "Regular" : "Start")), percent: discount.loyaltyPercent, completed_orders: cleanInt(body.completedOrders ?? currentExtras?.loyalty?.completed_orders, 10000) },
            ...
            if (action === "create") {
                ...
                if (!["pending", "waiting_payment", "confirmed"].includes(String(existing.status || "")))
        ...
        if (action === "update") {
            ...
            if (nextStatus === "confirmed" || nextStatus === "waiting_payment") {
                ...
                try {
                    ...
                    if (reservationError instanceof Error && reservationError.message === "inventory_conflict")
                        return json({ error: "inventory_conflict" }, 409);
                    throw reservationError;
                }
            }
            const allowed = nextStatus === "issued" ? ["confirmed"] : nextStatus === "completed" ? ["confirmed", "issued"] : ["pending", "waiting_payment", "confirmed", "issued"];
            if (!["issued", "completed", "declined", "cancelled"].includes(nextStatus) || !allowed.includes(current.status))
                return json({ error: "invalid_transition" }, 409);
            const now = new Date().toISOString(), patch = { status: nextStatus, admin_note: cleanText(body.adminNote, 800) || current.admin_note, updated_at: now };
            if (nextStatus === "waiting_payment")
                patch.hold_expires_at = new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString();
            else
            ...
            if (nextStatus === "issued")
                patch.issued_at = current.issued_at || now;
        ...
        if (action === "save_finance") {
            ...
            const calculationInput = { ...current, deposit_amount: depositAmount, deposit_paid: depositPaid, extras: { ...currentExtras, chemistry: { used_packets: usedPackets, story_mention: storyMention } } };
            const finance = settlementFromBooking(calculationInput, catalog, defaultCatalog), now = new Date().toISOString();
            const extras = { ...currentExtras, chemistry: { used_packets: finance.usedPackets, story_mention: finance.storyMention, free_packets: finance.freePackets, paid_packets: finance.paidPackets, price_per_packet: finance.packetPrice, amount: finance.chemistryAmount }, settlement: { ...(currentExtras.settlement || {}), model: "prepayment_plus_deposit", prepayment_amount: finance.prepaymentAmount, deposit_amount: finance.depositPaid ? finance.securityDeposit : 0, received_amount: finance.receivedAmount, expenses_amount: finance.totalAmount, refund_amount: finance.refundAmount, due_amount: finance.dueAmount, completed: false, completed_at: null, calculated_at: now } };
            const { data, error } = await supabase.from("vacleaner_bookings").update({ extras, extras_amount: finance.totalExtras, total_amount: finance.totalAmount, deposit_amount: depositAmount, deposit_paid: depositPaid, deposit_paid_at: depositPaid ? (current.deposit_paid_at || now) : null, issue_payment_amount: 0, issue_payment_paid: false, issue_payment_paid_at: null, updated_at: now }).eq("id", bookingId).select("*").single();
            if (error)
                throw error;
        ...
        if (action === "save_deposit_return") {
            ...
            if (currentError || !current)
                return json({ error: "invalid_booking" }, 404);
            if (current.deposit_paid !== true)
                return json({ error: "deposit_not_received" }, 409);
            if (!["confirmed", "issued", "completed"].includes(String(current.status || "")))
                return json({ error: "invalid_transition" }, 409);
            const finance = settlementFromBooking(current, catalog, defaultCatalog), confirmation = settlementConfirmation(body, finance);
            if (!confirmation.ok)
                return json({ error: confirmation.error, finance }, 409);
        ...
        if (message === "inventory_conflict")
            return json({ error: "inventory_conflict" }, 409);
        if (["invalid_dates", "invalid_exact_time", "invalid_rental_period"].includes(message))
