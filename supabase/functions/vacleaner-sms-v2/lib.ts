...
export const SMS_SENDER="VACLEANER",SMS_COOLDOWN_DAYS=90,SMS_OPT_OUT="vacleaner.pp.ua/s",SMS_LINK_TOKEN="{link}",SMS_LINK_SAMPLE="vacleaner.pp.ua/b#XXXXXXX",PERSONAL_PROMO_VALID_DAYS=21;
...
export async function preparePromoCodesForSend(db:any,campaign:any,rows:any[]){
  ...
  for(const row of rows){const current=byPhone.get(normalizePhone(row.phone));if(!current)throw new Error("promo_codes_missing");row.promoCode=String(current.code||row.promoCode||"");row.promoLink=promoShortLink(row.promoCode);row.promoCodeId=String(current.id||"");const {error:inactiveError}=await db.from("vacleaner_promo_codes").update({active:false,expires_at:null,activated_at:null,activation_source:null,activation_dispatch_id:null,activated_by:null}).eq("id",current.id);if(inactiveError)throw inactiveError}
  return rows;
}
