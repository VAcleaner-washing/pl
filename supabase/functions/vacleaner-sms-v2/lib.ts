...
export async function campaignPromoContext(db:any,campaignId:string,phones:string[]){
  ...
  const {data:campaign,error}=await db.from("vacleaner_campaigns").select("id,name,campaign_type,status,discount_type,discount_value,starts_at,ends_at").eq("id",campaignId).maybeSingle();if(error)throw error;
  ...
  const personalized=String(campaign.campaign_type||"")==="return";if(!personalized||!phones.length)return {personalized,campaign,byPhone:new Map<string,any>()};
  const {data:codes,error:codeError}=await db.from("vacleaner_promo_codes").select("id,code,customer_phone,active,expires_at").eq("campaign_id",campaignId).eq("active",true).in("customer_phone",phones);if(codeError)throw codeError;
  ...
  const byPhone=new Map<string,any>();for(const row of codes||[]){const phone=normalizePhone(row.customer_phone),link=promoShortLink(row.code),expired=row.expires_at&&new Date(row.expires_at).getTime()<=now;if(!phone||!link||expired||used.has(String(row.id)))continue;byPhone.set(phone,{promoCode:String(row.code),promoLink:link,promoCodeId:String(row.id)})}
...
export async function buildAudience(db:any,segment:string){
