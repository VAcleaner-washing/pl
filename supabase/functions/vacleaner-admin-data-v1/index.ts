  ...
  try{
    ...
    if(action==="campaigns"){
      const [{data:campaigns,error:campaignError},{data:codes,error:codesError},{data:redemptions,error:redemptionError},{data:dispatches,error:dispatchError}]=await Promise.all([
        ...
        db.from("vacleaner_promo_codes").select("id,campaign_id,code,customer_phone,active,expires_at,usage_limit,created_at,activated_at,activation_source,activation_dispatch_id").order("created_at",{ascending:false}).limit(1500),
    ...
    if(action==="create_campaign"){
      ...
      const {data:campaign,error}=await db.from("vacleaner_campaigns").insert({name,campaign_type:campaignType,status:"active",discount_type:discountType,discount_value:discountValue,dormant_days:dormantDays,allowed_product_codes:allowedProducts,allowed_weekdays:allowedWeekdays,min_completed_orders:campaignType==="return"?Math.max(1,minCompletedOrders):0,starts_at:startsAt,ends_at:endsAt,issuance_ends_at:["return","personal"].includes(campaignType)?endsAt:null,usage_limit_per_customer:1,created_by:userData.user.id}).select("*").single();if(error||!campaign)throw error||new Error("campaign_insert_failed");
      ...
      try{
        if(campaignType==="return"){
          ...
          codeRows=[{campaign_id:campaign.id,code,customer_phone:customerPhone,active:false,expires_at:null,usage_limit:1,activated_at:null,activation_source:null,activation_dispatch_id:null,activated_by:null}];const {error:codeError}=await db.from("vacleaner_promo_codes").insert(codeRows);if(codeError)throw codeError;
        }else{
