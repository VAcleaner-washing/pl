const CACHE='vacleaner-manager-native-v26-4247';
...
const CORE=[
  ...
  '/admin/manifest-native-v26.webmanifest',
  '/admin/icon-192.png','/admin/icon-512.png','/admin/apple-touch-icon.png?v=4247'
];
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE)).then(()=>self.skipWaiting())));
self.addEventListener('activate',event=>event.waitUntil(Promise.all([
  caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('vacleaner-manager-native-v26-')&&key!==CACHE).map(key=>caches.delete(key)))),
  self.clients.claim(),
])));
