const CACHE='vacleaner-manager-native-v27-4247';
...
const CORE=[
  ...
  '/assets/admin-native-v26.css?v=4247',
  '/assets/admin-native-v26.js?v=4247',
  '/assets/admin-native-v27.css?v=4247',
  '/assets/admin-native-v27.js?v=4247',
  '/admin/manifest-native-v27.webmanifest',
  '/admin/icon-192.png','/admin/icon-512.png','/admin/apple-touch-icon.png?v=4247'
];
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE)).then(()=>self.skipWaiting())));
self.addEventListener('activate',event=>event.waitUntil(Promise.all([
  caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('vacleaner-manager-native-v27-')&&key!==CACHE).map(key=>caches.delete(key)))),
  self.clients.claim(),
])));
