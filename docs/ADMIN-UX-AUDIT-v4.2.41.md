...
9. На wide desktop спільний перемикач періоду більше не стискається в toolbar Аналітики: п’ять опцій залишаються в одному рядку й мають ту саму геометрію, що у Фінансах.
...
- v4.2.41 interaction contract: 13/13 pass;
- Finance/Analytics control consistency: 110/110 pass across 390 / 430 / 1024 / 1280 / 1440 px, including relative X/Y, container size and one-row desktop geometry;
- desktop synthetic-data inspection: booking list, navigation, calendar and clients list reviewed;
- canonical CI evidence supplied before this patch: 30/31 browser suites passed and the only failure was `test:admin-control-consistency` at 1440 px;
- after the patch, the affected suite and adjacent admin suites (PWA 897/897, typography 185/185, context navigation 23/23, route smoke 22/22, Glass, desktop density and final desktop visual) pass locally. A fresh canonical GitHub aggregate remains the production gate.
...
v4.2.41 is a low-risk interaction/accessibility release. Business rules and data contracts are preserved. Static gates and the corrected Finance/Analytics geometry gate are green. Production promotion still requires one fresh canonical GitHub browser/PWA aggregate for this exact archive.
