(()=>{
  'use strict';
  const ROUTE='/admin/bronuvannia-native-v27/';
  ...
  function patchLocalNotification(){
    if(typeof window.sendLocalNotification!=='function'||window.sendLocalNotification.__nativeV27)return;
    ...
    fn.__nativeV27=true;window.sendLocalNotification=fn;
  }

  function registerScopedSw(){
    if(!('serviceWorker' in navigator)||!location.pathname.startsWith(ROUTE))return;
    navigator.serviceWorker.register('/admin/sw-native-v27.js?v=4247',{scope:ROUTE}).then(reg=>reg.update?.()).catch(err=>console.warn('native_v27_sw_failed',err));
  }

  function apply(){
    if(!mobile())return;
    document.documentElement.classList.add('native-v25','native-v26','native-v27');
