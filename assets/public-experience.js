(()=>{
  ...
  function enhanceDepositSummary(){
    ...
    row.innerHTML='<span>Залог при видачі<small>базова сума; фактичну фіксує менеджер</small></span><strong>—</strong>';
    ...
    let pre=summary.querySelector('.vx-summary-prepayment');if(!pre){pre=document.createElement('div');row.insertAdjacentElement('beforebegin',pre)}
    pre.className='vx-summary-prepayment';pre.innerHTML='<span>Бронювання дати<small>сплачується після підтвердження</small></span><strong>200 грн</strong>';
    summary.querySelectorAll('.vx-summary-deposit-note').forEach((el,index)=>{if(index)el.remove()});
    let note=summary.querySelector('.vx-summary-deposit-note');if(!note){note=document.createElement('p');row.insertAdjacentElement('afterend',note)}
    note.className='vx-summary-deposit-note';note.textContent='При поверненні із 200 грн передоплати та фактично внесеного залогу віднімаються оренда, доставка, додаткові засоби й використана хімія. Решту повертаємо або клієнт доплачує різницю.';
  }
  ...
  function termsMarkup(){
    return `<section class="vx-rental-terms" data-vx-rental-terms="${VERSION}" aria-labelledby="vx-rental-terms-title"><div class="vx-rental-terms__inner"><div class="vx-rental-terms__head"><p>Умови оренди · без прихованих платежів</p><h2 id="vx-rental-terms-title">Що потрібно для бронювання</h2><span>Передоплата та фактичний залог формують спільний фінальний розрахунок при поверненні.</span></div><div class="vx-rental-steps"><article><b>01</b><div><h3>Передплата 200 грн</h3><p>Вноситься після підтвердження заявки, закріплює дату та повністю входить у вартість оренди.</p><dl><div><dt>ФОП</dt><dd>Невідома Анна Сергіївна</dd></div><div><dt>IBAN</dt><dd>UA523220010000026006370119233</dd></div><div><dt>ІПН</dt><dd>3314215243</dd></div><div><dt>Призначення</dt><dd>сплата за оренду техніки</dd></div></dl></div></article><article><b>02</b><div><h3>Документ для договору</h3><p>Новий клієнт надсилає фото паспорта, ID-картки або водійського посвідчення менеджеру приватно. Якщо ви вже орендували техніку й дані є в базі — повторно надсилати документ не потрібно.</p></div></article><article><b>03</b><div><h3>Залог при передачі</h3><p>Базова сума підставляється автоматично, але менеджер фіксує фактично отриману суму при видачі. Після повернення з передоплати та залогу віднімаються оренда, доставка, додаткові засоби й використана хімія; решта повертається клієнту або він доплачує різницю.</p><div class="vx-deposit-table"><span><b>1 одиниця</b><em>1 000 грн</em><small>повний вікенд · 2 000 грн</small></span><span><b>2 одиниці / комплект</b><em>1 500 грн</em><small>повний вікенд · 3 000 грн</small></span><span><b>Генеральне</b><em>2 000 грн</em><small>повний вікенд · 3 000 грн</small></span><span><b>HOME RESET</b><em>3 000 грн</em><small>повний вікенд · 4 000 грн</small></span></div></div></article></div><p class="vx-rental-terms__privacy">Номери документів зберігаються у закритій базі VAcleaner лише для оформлення договорів і не показуються на публічному сайті.</p></div></section>`;
  }
