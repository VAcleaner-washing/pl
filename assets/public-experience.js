(()=>{
  ...
  const FALLBACK_ALIASES={'Kärcher Puzzi':'puzzi','Kärcher Puzzi 8/1':'puzzi','Puzzi + Jimmy':'puzzi_jimmy','Глибоке очищення':'puzzi_jimmy','Глибоке очищення текстилю':'puzzi_jimmy','Глибоке очищення диванів і матраців':'puzzi_jimmy','Puzzi + робот для вікон':'puzzi_abir','Puzzi + робот ABIR':'puzzi_abir','Текстиль + вікна':'puzzi_abir','Дивани + вікна':'puzzi_abir','Kärcher SC 2':'sc2','Kärcher SC 2 Deluxe':'sc2','Робот для вікон':'abir','Робот ABIR':'abir','Комбо':'combo','Тариф «Комбо»':'combo','Комбо · Puzzi + SC 2':'combo','Текстиль + кухня та ванна':'combo','Дивани + кухня та ванна':'combo','Генеральне':'general','Генеральне прибирання':'general','Ідеальні вікна':'ideal_windows','Вікна та гладкі поверхні':'ideal_windows','HOME RESET':'elite','Весь дім за один вікенд':'elite','Весь дім · HOME RESET':'elite'};
  ...
  const DEFAULT_SLOTS=clone(CORE?.slots||{morningStart:'08:00',morningEnd:'10:00',eveningStart:'17:30',eveningEnd:'20:00'});
  ...
  let liveSlots=clone(DEFAULT_SLOTS);
  ...
  function enhanceDepositSummary(){
    ...
    const prepayment=ensureDepositSummaryRow(summary,total,'vx-summary-prepayment','Передплата','Сплачуєте після підтвердження. Входить у вартість.');
    setTextIfChanged(prepayment.querySelector('strong'),'200 грн');
    const deposit=ensureDepositSummaryRow(summary,total,'vx-summary-deposit','Залоговий платіж','Сплачуєте при отриманні. Після розрахунку повертаємо залишок.');
  ...
  function syncBookingCatalog(){
    ...
    document.querySelectorAll('.booking-products button').forEach(button=>{
      ...
      const code=PRODUCT_ALIASES[strong.textContent.trim()]||CORE?.productAliases?.[strong.textContent.trim()]||Object.entries(PUBLIC_PRODUCT_LABELS).find(([,label])=>label===strong.textContent.trim())?.[0];
    ...
    if(buttons.length&&!buttons.some(button=>/Дивани \+ вікна/.test(button.textContent||''))){
      ...
      if(robot&&product){
        ...
        setTextIfChanged(button.querySelector('span'),'Puzzi + робот для вікон · м’які меблі, матраци, вікна й дзеркала');
  ...
  function syncPackageCatalog(){
    ...
    const combo=[...grid.querySelectorAll('.package-card')].find(card=>/Дивани \+ кухня та ванна|Текстиль \+ кухня та ванна|^Комбо$/m.test(card.querySelector('h2')?.textContent.trim()||''));
    if(combo)setTextIfChanged(combo.querySelector('h2'),PUBLIC_PRODUCT_LABELS.combo);
    if([...grid.querySelectorAll('.package-card h2')].some(h=>h.textContent.trim()==='Дивани + вікна'))return;
  ...
  function syncPublicSettings(){
    if(location.pathname.replace(/\/+$/,'')!=='/tekhnika/karcher-puzzi-8-1')return;
    document.querySelectorAll('.puzzi-term-grid article').forEach(card=>{
      if(card.querySelector('small')?.textContent.trim()!=='Слоти')return;
      setTextIfChanged(card.querySelector('p'),`Ранок ${liveSlots.morningStart}–${liveSlots.morningEnd} · вечір ${liveSlots.eveningStart}–${liveSlots.eveningEnd}. Доступність техніки перевіряється онлайн перед бронюванням.`);
    });
  }
  async function loadDepositRules(){try{const r=await fetch(SETTINGS_API,{cache:'no-store'}),d=await r.json();if(r.ok){if(d.depositRules)mergeDepositRules(d.depositRules);if(d.deliveryFee!==undefined)setDeliveryFee(d.deliveryFee);if(d.catalog?.products)liveCatalog=d.catalog;const slots=validSlots(d.slots);if(slots)liveSlots=slots;syncBookingCatalog();syncPublicSettings();enhanceDepositSummary();syncDeliveryFee();syncTermsDepositRules()}}catch{}}
  function depositRuleMarkup(group,label){
    const rule=depositRules[group]||DEFAULT_DEPOSIT_RULES[group]||DEFAULT_DEPOSIT_RULES.oneUnit;
    return `<span data-vx-deposit-rule="${group}"><b>${label}</b><em>${formatMoney(rule.day)}</em><small>2+ доби у вікенд · ${formatMoney(rule.weekend)}</small></span>`;
  }
  function syncTermsDepositRules(){
    const root=document.querySelector('[data-vx-rental-terms]');if(!root)return;
    const labels={oneUnit:'1 одиниця',twoUnits:'2 одиниці / комплект',general:'Генеральне',elite:'HOME RESET'};
    Object.entries(labels).forEach(([group,label])=>{
      const row=root.querySelector(`[data-vx-deposit-rule="${group}"]`),rule=depositRules[group]||DEFAULT_DEPOSIT_RULES[group]||DEFAULT_DEPOSIT_RULES.oneUnit;if(!row)return;
      setTextIfChanged(row.querySelector('b'),label);setTextIfChanged(row.querySelector('em'),formatMoney(rule.day));setTextIfChanged(row.querySelector('small'),`2+ доби у вікенд · ${formatMoney(rule.weekend)}`);
    });
  }
  function termsMarkup(){
    return `<section class="vx-rental-terms" data-vx-rental-terms="${VERSION}" aria-labelledby="vx-rental-terms-title"><div class="vx-rental-terms__inner"><div class="vx-rental-terms__head"><p>Умови оренди · без прихованих платежів</p><h2 id="vx-rental-terms-title">Що потрібно для оформлення</h2><span>Передплата входить у вартість оренди. Залоговий платіж сплачується окремо при отриманні та враховується під час остаточного розрахунку.</span></div><div class="vx-rental-steps"><article><b>01</b><div><h3>Передплата 200 грн</h3><p>Сплачуєте тільки після підтвердження заявки. Вона закріплює дату та входить у вартість оренди.</p><dl><div><dt>ФОП</dt><dd>Невідома Анна Сергіївна</dd></div><div><dt>IBAN</dt><dd>UA523220010000026006370119233</dd></div><div><dt>ІПН</dt><dd>3314215243</dd></div><div><dt>Призначення</dt><dd>сплата за оренду техніки</dd></div></dl></div></article><article><b>02</b><div><h3>Документ для договору</h3><p>Новий клієнт надсилає фото паспорта, ID-картки або водійського посвідчення менеджеру приватно. Якщо ви вже орендували техніку й дані є в базі — повторно надсилати документ не потрібно.</p></div></article><article><b>03</b><div><h3>Залоговий платіж</h3><p>Сплачуєте при отриманні техніки. Після повернення робимо остаточний розрахунок і повертаємо невикористаний залишок залогового платежу.</p><div class="vx-deposit-table">${depositRuleMarkup('oneUnit','1 одиниця')}${depositRuleMarkup('twoUnits','2 одиниці / комплект')}${depositRuleMarkup('general','Генеральне')}${depositRuleMarkup('elite','HOME RESET')}</div></div></article></div><section class="vx-loyalty-policy" aria-label="Програма лояльності"><div><p>Програма лояльності</p><h3>Чим більше оренд — тим вигідніше.</h3><span>Знижка застосовується автоматично за номером телефону та діє тільки на оренду техніки.</span></div><div class="vx-loyalty-levels"><article><small>Start</small><strong>0%</strong><span>0–2 завершені оренди</span></article><article><small>Regular</small><strong>−5%</strong><span>після 3 завершених оренд</span></article><article><small>VIP</small><strong>−10%</strong><span>після 6 завершених оренд</span></article></div><p class="vx-loyalty-rule">Promo та loyalty не сумуються — система автоматично застосовує вигіднішу знижку. Доставка, додаткові позиції та хімія оплачуються без знижки.</p></section><section class="vx-care-policy" aria-label="Дбайливе користування технікою"><div class="vx-care-policy__head"><p>Дбайливе користування</p><h3>Без страшилок і дрібного шрифту.</h3><span>Техніка видається перевіреною та справною. Якщо щось працює не так — зупиніть роботу й напишіть нам.</span></div><div class="vx-care-policy__grid"><article><small>01</small><strong>Несправність</strong><span>Природний знос або технічна несправність не з вини клієнта — не його відповідальність. Самостійно розбирати чи ремонтувати техніку не потрібно.</span></article><article><small>02</small><strong>Фізичні пошкодження</strong><span>Відповідальність виникає за очевидні пошкодження через неправильне користування: падіння, удари, тріщини, залиття або самостійне розбирання.</span></article><article><small>03</small><strong>Повернення</strong><span>Злийте брудну воду, приберіть велике сміття й волосся, сполосніть робочі ємності та насадки. Звичайні сліди використання — нормально.</span></article></div><p class="vx-care-policy__proof">300 оренд — і нам ще не доводилося штрафувати клієнтів за техніку.</p></section><p class="vx-rental-terms__privacy">Номери документів зберігаються у закритій базі VAcleaner лише для оформлення договорів і не показуються на публічному сайті.</p></div></section>`;
  }
...
;(()=>{
  const loadBookingHardening=()=>{
    ...
    if(!document.querySelector('link[href*="booking-hardening-v4144.css"]')){
      const link=document.createElement('link');link.rel='stylesheet';link.href='/assets/booking-hardening-v4144.css?v=4155';document.head.appendChild(link);
    }
    if(!document.querySelector('script[src*="booking-hardening-v4144.js"]')){
      const script=document.createElement('script');script.src='/assets/booking-hardening-v4144.js?v=4155';script.defer=true;document.head.appendChild(script);
    }
  };
