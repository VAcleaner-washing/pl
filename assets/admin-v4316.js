/* VAcleaner v4.3.16 — compact exact-time range 07:00–21:00.
   Keeps canonical pickupTime/returnTime values and the 30-minute admin grid,
   while limiting new admin choices to the operational daytime range. */
(() => {
  'use strict';
  ...
  for (let total = START_MINUTES; total <= END_MINUTES; total += STEP_MINUTES) {
  ...
  function isSelectableTime(value) {
    const total = minutesOf(value);
    return Number.isFinite(total)
      && total >= START_MINUTES
      && total <= END_MINUTES
      && total % STEP_MINUTES === 0;
  }
  ...
  function openSheet(input, trigger) {
    ...
    if (!isSelectableTime(current)) grid.appendChild(makeOption(current, current, true));
  ...
  function upgrade(input) {
    if (!(input instanceof HTMLInputElement) || input.dataset.v4316Time === '1') return;
    if (!['pickupTime', 'returnTime'].includes(input.name)) return;
    const wrapper = input.closest('.admin-exact-time-picker');
    if (!wrapper) return;

    input.dataset.v4316Time = '1';
