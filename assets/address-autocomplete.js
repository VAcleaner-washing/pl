...
function selectItem(ctx,index){
  ...
  if(item.houseNumber){ctx.input.dataset.vacAddressSelected='1';setStatus(ctx,'ok','Адресу знайдено. Можна додати квартиру або під’їзд нижче.');closeList(ctx);ctx.details?.focus()}
...
async function search(ctx){
  ...
  if(q.length<3){closeList(ctx);setStatus(ctx,'hint','Почніть вводити вулицю — підкажемо адреси Полтави.');return}
...
function attach(input,mode){
  ...
  detailBox.innerHTML='<span>Квартира / під’їзд / поверх <small>необов’язково</small></span><input type="text" data-vac-address-detail="1" maxlength="120" autocomplete="off" placeholder="Наприклад: кв. 24, 2 під’їзд, 5 поверх" aria-label="Квартира, під’їзд, поверх або примітка для доставки">';
  ...
  setStatus(ctx,'hint','Почніть вводити вулицю — підкажемо адреси Полтави.');
