(()=>{
  ...
  const PRODUCT_INFO={
    puzzi:{label:'Kärcher Puzzi',price:700,desc:'Глибоке промивання диванів, крісел, матраців і текстилю.'},
    puzzi_jimmy:{label:'Глибоке очищення текстилю',price:1050,desc:'Puzzi + Jimmy · сухий етап і глибоке промивання диванів, матраців, крісел та домашнього текстилю.'},
  ...
  const EXTRA_EXPLANATION={
    ...
    stain_exit:{why:'Кава, чай, вино, ягоди, соки та інші харчові продукти можуть залишити органічний кольоровий слід після звичайного очищення.',result:'Кислотний плямовивідник pH 3,5 працює саме з такими локальними органічними плямами.'},
  ...
  const SPOT_FIX_USE='Не розбавляйте. Для невеликої ділянки нанесіть засіб на чисту білу тканину та акуратно протріть забруднення. Для більшої площі розпиліть засіб. Потім промокніть — не тріть — чистою сухою білою тканиною, щоб прибрати зайву вологу. Завершіть промиванням водою або очищенням усієї поверхні миючим засобом із подальшим відбиранням вологи Puzzi.';
  const STAIN_OX_USE='Не розбавляйте. Спочатку перевірте сумісність на непомітній ділянці: нанесіть трохи засобу й промокніть білою бавовняною тканиною. Якщо на тканину перейшов колір матеріалу — засіб не використовуйте. Нанесіть безпосередньо на пляму, залиште діяти до 15 хвилин, після чого промийте холодною водою та відберіть вологу Puzzi.';
  ...
  function questions(){
    ...
    if(hasTextile())list.push({id:'textileProblems',title:'Що є на текстилі?',note:'Позначте все, що бачите — так підберемо і техніку, і потрібні засоби.',type:'multi',options:[
      ['common_stain','Їжа, жир, косметика або невідома пляма','Підберемо універсальний плямовивідник'],['color_stain','Кава, чай, вино, ягоди або сік','Для кольорових органічних слідів'],['odor','Неприємний запах','Сеча, домашні тварини або інші стійкі запахи'],['dry_debris','Шерсть, волосся або багато пилу','Додамо сухе очищення перед промиванням'],['none','Нічого з цього','Потрібне лише загальне очищення']
    ]});
  ...
  function injectStainCareSection(){
    ...
    if(stainOx){stainOx.querySelector('p').textContent=STAIN_OX_USE;stainOx.querySelector('small').textContent='Кислотний засіб, pH 3,5. Використовуйте засоби індивідуального захисту, щонайменше рукавички. Для особливо застарілої плями обробку можна повторити лише після повного промивання поверхні.';}
    target.insertAdjacentElement('beforebegin',section);
