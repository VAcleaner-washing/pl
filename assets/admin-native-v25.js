(()=>{
  ...
  function patchLocalNotification(){
    if(typeof window.sendLocalNotification!=='function'||window.sendLocalNotification.__nativeV25)return;
    ...
    fn.__nativeV25=true;window.sendLocalNotification=fn;
  }

  function registerScopedSw(){
    if(!('serviceWorker' in navigator)||!location.pathname.startsWith(ROUTE))return;
    navigator.serviceWorker.register('/admin/sw-native-v25.js?v=4247',{scope:ROUTE}).then(reg=>reg.update?.()).catch(err=>console.warn('native_v25_sw_failed',err));
  }

  function apply(){
    if(!mobile())return;
    document.documentElement.classList.add('native-v25');
